...
class ProgressIndicatorMixin:
    ...
    def get(self, *args, **kwargs):
        if constants.SESSION_KEY_INGRESS_ANON not in self.storage.extra_data and (
...
class CreateUserAccountMixin:
    ...
    def user_account_condition(self):
        ...
        # device
        skipped_first_step = self.kwargs['step'] == (
            constants.VERIFICATION and constants.USER_ACCOUNT not in self.storage.data['step_data']
        )
