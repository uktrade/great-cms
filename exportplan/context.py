...
class ExportPlanDashboardPageContextProvider(AbstractPageContextProvider):
    ...
    def get_context_data(request, page):
        return {
            'sections': helpers.build_export_plan_sections(request.user.export_plan.data),
            'export_plan_progress': helpers.calculate_ep_progress(request.user.export_plan.data),
        }
