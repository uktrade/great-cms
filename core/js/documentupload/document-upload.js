export const documentUpload = () => {
  const isAddMultipleDocumentPage = utils.isAddMultipleDocumentPage(window.location.pathname)
  if(isAddMultipleDocumentPage){
    const uploadFileInput = document.querySelector('#fileupload');
    $(function () {
      $('#fileupload').fileupload({
        ...
        add: function (e, data) {
          ...
          //check the size and retirn failure
  ...
  function setup() {
    const MAX_FILE_SIZE = 20971520;  // 20 MB in bytes
    uploadFileInput.addEventListener('click', (event) => {
      event.preventDefault()
      file = event.target.files[0]

      if (file) {
        if (file.size > MAX_FILE_SIZE) {
          alert("File is too large! Please upload a file smaller than 20 MB.");
          return;
        }
