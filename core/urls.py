...
from core import views, views_api
...
urlpatterns = [
    path(
        'markets/',
        views.MarketsView.as_view(),
        name='markets'
    ),
    path('find-your-target-market/',
         login_required(views.TargetMarketView.as_view(), login_url=LOGIN_URL),
         name='target-market'
         ),
    path(
    ...
    path(
        'login/',
        anonymous_user_required(views.LoginView.as_view()),
        name='login'
    ),
    path(
        'signup/',
        anonymous_user_required(views.SignupView.as_view()),
        name='signup'
    ),
    path(
        'signup/company-name/',
    ...
    path(
        ...
        anonymous_user_required(
            skip_ga360(
                views.SignupForTailoredContentWizardView.as_view(url_name='core:signup-wizard-tailored-content'))
        ),
    ...
    path(
        'contact-us/help/',
        skip_ga360(views.ContactUsHelpFormView.as_view()),
        name='contact-us-help'
    ),
    path(
        'contact-us/success/',
        skip_ga360(views.ContactUsHelpSuccessView.as_view()),
        name='contact-us-success'
    ),
    path(
        'api/update-company/',
        skip_ga360(views_api.UpdateCompanyAPIView.as_view()),
        name='api-update-company'
    ),
    path(
        'api/lookup-product/',
        skip_ga360(views_api.ProductLookupView.as_view()),
        name='api-lookup-product'
    ),
    path(
        'api/countries/',
        skip_ga360(views_api.CountriesView.as_view()),
        name='api-countries'
    ),
    path(
    ...
    path(
        'api/create-token/',
        skip_ga360(views_api.CreateTokenView.as_view()),
        name='api-create-token'
    ),
    path(
        'api/check/',
        skip_ga360(views_api.CheckView.as_view()),
        name='api-check'
    ),
    path(
        'api/data-service/comtrade/',
        skip_ga360(views_api.ComTradeDataView.as_view()),
        name='api-comtrade-data'
    ),
