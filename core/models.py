...
from core import blocks as core_blocks, cms_panels, constants, forms, mixins, snippet_slugs
...
class DetailPage(settings.FEATURE_DEA_V2 and CMSGenericPageAnonymous or CMSGenericPage):
    ...
    def serve(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            form = forms.CsatUserFeedbackForm(request.POST)
            if form.is_valid():
                return self.form_valid(request, form)
            else:
                return self.form_invalid(request, form)
        else:
            initial_data = self.get_initial(request)
            form = forms.CsatUserFeedbackForm(initial=initial_data)
            return render(request, self.template_choices, {'form': form, 'context': self.get_context_data(request)})

    def get_csat(self):
        csat_id = self.request.session('learn_to_export_csat_id')
        if csat_id:
    ...
    def get_context_data(self, request, **kwargs):
        stage = request.session.get('learn_to_export_csat_stage', 0)
    ...
    def form_invalid(self, request, form):
        ...
        js_enabled = 'js_enabled' in request.get_full_path()
        if js_enabled:
