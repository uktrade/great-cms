...
def handler404(request, *args, **kwargs):
    if helpers.international_url(request) in request.path:
...
class ContactView(WagtailCacheMixin, GA360Mixin, FormView):  # /PS-IGNORE
    ...
    template_name = 'core/business_growth_contact.html'
    subject = 'Business.gov.uk contact form'
    ...
    def get_back_url(self):
        back_url = '/'
        if self.request.GET.get('next'):
            back_url = helpers.check_url_host_is_safelisted(self.request)
        return back_url

    def get_success_url(self):
        success_url = reverse_lazy('core:business-contact-sent')
        if self.request.GET.get('next'):
            success_url = success_url + '?next=' + helpers.check_url_host_is_safelisted(self.request)
        return success_url
    ...
    def submit_feedback(self, form):
        ...
        if self.request.GET.get('next'):
            cleaned_data['from_url'] = helpers.check_url_host_is_safelisted(self.request)
        ...
        action = actions.ZendeskAction(
            ...
            service_name='great',
            form_url=reverse('core:business-contact'),
            sender=sender,
        )
...
class ContactSuccessView(WagtailCacheMixin, HCSATMixin, FormView, GA360Mixin, TemplateView):  # /PS-IGNORE
    template_name = 'core/business_growth_contact_sent.html'
    subject = 'Business.gov.uk contact form success'
    hcsat_service_name = 'business_support'
    cache_control = 'no-cache'
    form_class = forms.HCSATForm
