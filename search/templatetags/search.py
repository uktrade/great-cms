...
def format_for_results_list(search_results):
    ...
    for result in search_results:
        formatted_result = {
            ...
            'description': (result.search_description or 
                           getattr(result.specific, 'seo_description', '') or
                           getattr(result.specific, 'search_description', '') or
                           getattr(result.specific, 'featured_description', '') or
                           getattr(result.specific, 'heading_teaser', '') or
                           getattr(result.specific, 'teaser', '') or
                           getattr(result.specific, 'page_teaser', '') or
                           getattr(result.specific, 'page_subheading', '') or
