...
class FeedbackForm(SaveOnlyInDatabaseAPIForm):
    ...
    search_target = gds_forms.CharField(
        label='Whether yes or no, please let us know what you were searching for',
        widget=Textarea(attrs={'rows': 4, 'cols': 15}),
    )
    from_search_query = gds_forms.CharField(widget=HiddenInput(), required=False)
    from_search_page = IntegerField(widget=HiddenInput(), required=False)
    contactable = gds_forms.ChoiceField(
        label='May we contact you with some brief follow-up questions on your experience?',
        widget=gds_forms.RadioSelect(),
        choices=[('yes', 'Yes'), ('no', 'No')],
    )
    contact_name = gds_forms.CharField(label='What is your name?', required=False)
    contact_email = gds_forms.EmailField(label='What is your email address?', required=False)
    contact_number = gds_forms.CharField(label='What is your phone number? (optional)', required=False)
