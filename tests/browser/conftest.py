...
def mock_update_company_profile():
    with patch.object(
        core_helpers, 'update_company_profile', return_value=create_response()
    ) as patched:
        yield patched
...
def mock_get_dashboard_export_opportunities():
    with patch.object(
        core_helpers, 'get_dashboard_export_opportunities', return_value=[]
    ) as patched:
        yield patched
...
def mock_get_export_plan_market_data():
    ...
    with patch.object(
        exportplan_helpers, 'get_exportplan_marketdata', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_export_plan_rules_regulations():
    ...
    with patch.object(
        exportplan_helpers,
        'get_exportplan_rules_regulations',
...
def mock_get_comtrade_last_year_import_data():
    ...
    with patch.object(
        exportplan_helpers, 'get_comtrade_lastyearimportdata', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_exportplan():
    ...
    with patch.object(
        exportplan_helpers, 'get_exportplan', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_recommended_countries():
    ...
    with patch.object(
        exportplan_helpers, 'get_recommended_countries', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_export_plan_list():
    ...
    with patch.object(
        api_client.exportplan, 'exportplan_list', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_ease_of_doing_business():
    ...
    with patch.object(
        api_client.dataservices, 'get_easeofdoingbusiness', return_value=return_value
    ) as patched:
        yield patched
...
def mock_get_last_year_import_data():
    ...
    with patch.object(
        api_client.dataservices, 'get_lastyearimportdata', return_value=return_value
    ) as patched:
        yield patched
...
def mock_update_export_plan():
    return_value = {}
    with patch.object(
        exportplan_helpers, 'update_exportplan', return_value=return_value
    ) as patched:
        yield patched
