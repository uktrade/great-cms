...
def test_personal_profile_edit_existing_profile(client, mock_update_user_profile, sso_user_with_profile):
    client.force_login(sso_user_with_profile)
    ...
    assert mock_update_user_profile.call_args == mock.call(
        sso_session_id=sso_user_with_profile.session_id,
        data={'first_name': 'Foo', 'last_name': 'Example', 'job_title': 'Exampler', 'mobile_phone_number': '1232342'},
    )


def test_personal_profile_edit_no_profile(client, mock_create_user_profile, sso_user_no_profile):
