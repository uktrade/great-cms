...
@pytest.mark.django_db
def test_set_lesson_pages_topic_id(rf, topics_with_lessons_and_placeholders):

    request = rf.get('/')
    request.user = AnonymousUser()

    # Rest the topic_block_id for all lessons
    curated_page = topics_with_lessons_and_placeholders[0][0]
    for lesson_page in topics_with_lessons_and_placeholders[0][1]:
...
@pytest.mark.django_db
def test_set_lesson_pages_topic_id_removed(rf, topics_with_lessons_and_placeholders):
