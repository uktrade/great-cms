...
def test_retrieve_marketing_target_age_data(mock_population_data, client, user):
    client.force_login(user)

    mock_population_data.return_value = {'population_data': {'target_population': 10000}}

    url = reverse('exportplan:api-marketing-country-age-group-data')
    ...
    assert response.json() == {'population_data': {'target_population': 10000}}
