...
export class ObjectivesList extends Component {
  constructor(props) {
    ...
    afterSave$.subscribe((data) => {
      this.setState((state) => {
        const objectiveToUpdate = state.objectives.indexOf(
          state.objectives.filter(
            (objective) => objective.pk === data.data.pk
          )[0]
        )
        const updatedObjectives = [...state.objectives]
  ...
  handleUpdateSuccess = (response) => {
    response.json().then((data) => {
      this.setState((state) => {
        const objectiveToUpdate = state.objectives.indexOf(
          state.objectives.filter((objective) => objective.pk === data.pk)[0]
        )
        const updatedObjectives = [...state.objectives]
  ...
  handleError = (errors, pk) => {
    this.setState((state) => {
      const objectiveToUpdate = state.objectives.indexOf(
        state.objectives.filter((objective) => objective.pk === pk)[0]
      )
      const updatedObjectives = [...state.objectives]
