  ...
  if (comparisonMarkets && Object.keys(comparisonMarkets).length) {
    ...
    const tableBody = Object.values(comparisonMarkets).map(market => {
      ...
      return (<tr key={`market-${market.country_iso2_code}`} id={`market-${market.country_name}`}>
        <td className="p-v-xs name"><span className="body-l-b" id={`market-${market.country_name}`}>{market.country_name}</span><button type="button" onClick={removeMarket} className="iconic" data-id={market.country_iso2_code} aria-label={`Remove ${market.country_name}`}><i className="fa fa-times-circle"/></button></td>
