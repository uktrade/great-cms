            ...
            if field_name in fields:
                ...
                try:
                    ...
                    raise BadRequestError(
                        "field filter error. '%s' is not a valid value for %s (%s)" % (value, field_name, str(e))
                    )
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if "search" in request.GET:
            ...
            try:
                ...
                raise BadRequestError(
                    "cannot filter by '{}' while searching (field is not indexed)".format(e.field_name)
                )
            except OrderByFieldError as e:
                raise BadRequestError(
                    "cannot order by '{}' while searching (field is not indexed)".format(e.field_name)
                )
