...
class Elasticsearch5SearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def _compile_fuzzy_query(self, query, fields):
        if self.partial_match:
            ...
            raise NotImplementedError(
                "Fuzzy search on multiple fields is not supported by the " "Elasticsearch search backend."
            )
        return {
            "match": {
                fields[0]: {
    ...
    def _compile_query(self, query, field, boost=1.0):
        if isinstance(query, MatchAll):
            ...
            return {
                "bool": {"must": [self._compile_query(child_query, field, boost) for child_query in query.subqueries]}
            }

        elif isinstance(query, Or):
            return {
                "bool": {"should": [self._compile_query(child_query, field, boost) for child_query in query.subqueries]}
            }
            ...
            raise NotImplementedError(
                "`%s` is not supported by the Elasticsearch search backend." % query.__class__.__name__
            )
