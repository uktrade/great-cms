...
class Index:
    def __init__(self, backend, db_alias=None):
        ...
        if self.connection.vendor != "mysql":
            raise NotSupportedError("You must select a MySQL database " "to use MySQL search.")
...
class MySQLSearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def build_search_query_content(self, query, invert=False):
        if isinstance(query, PlainText):
            ...
            subquery_lexemes = [
                self.build_search_query_content(subquery, invert=invert) for subquery in query.subqueries
            ]
        ...
        raise NotImplementedError("`%s` is not supported by the MySQL search backend." % query.__class__.__name__)
    ...
    def search(self, config, start, stop, score_field=None):
        ...
        score_expression = MatchExpression(search_query, columns=["title"], output_field=FloatField()) * F(
            "title_norm"
