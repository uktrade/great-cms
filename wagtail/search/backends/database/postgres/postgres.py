...
class Index:
    def __init__(self, backend, db_alias=None):
        ...
        if self.connection.vendor != "postgresql":
            raise NotSupportedError("You must select a PostgreSQL database " "to use PostgreSQL search.")
    ...
    def _refresh_title_norms(self, full=False):
        ...
        entries.annotate(title_length=Length("title")).filter(title_length__gt=0).update(
            title_norm=lavg / F("title_length")
        )

    def delete_stale_model_entries(self, model):
        existing_pks = (
            model._default_manager.using(self.db_alias).annotate(object_id=Cast("pk", TextField())).values("object_id")
        )
        content_types_pks = get_descendants_content_types_pks(model)
        stale_entries = self.entries.filter(content_type_id__in=content_types_pks).exclude(object_id__in=existing_pks)
    ...
    def add_items_upsert(self, content_type_pk, indexers):
        ...
        data_sql = ", ".join(
            ["(%%s, %%s, %s, %s, %s, 1.0)" % (a, b, c) for a, b, c in zip(title_sql, autocomplete_sql, body_sql)]
        )
    ...
    def add_items(self, model, objs):
        ...
        if indexers:
            ...
            update_method = self.add_items_upsert if self._enable_upsert else self.add_items_update_then_create
...
class PostgresSearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def build_tsquery_content(self, query, config=None, invert=False):
        if isinstance(query, PlainText):
            ...
            subquery_lexemes = [
                self.build_tsquery_content(subquery, config=config, invert=invert) for subquery in query.subqueries
            ]
        ...
        raise NotImplementedError("`%s` is not supported by the PostgreSQL search backend." % query.__class__.__name__)
    ...
    def build_tsrank(self, vector, query, config=None, boost=1.0):
        if isinstance(query, (Phrase, PlainText, Not)):
            ...
            return (
                MUL(
                    1 + self.build_tsrank(vector, subquery, config=config, boost=boost) for subquery in query.subqueries
                )
                - 1
            )
            ...
            return ADD(
                self.build_tsrank(vector, subquery, config=config, boost=boost) for subquery in query.subqueries
    ...
    def _build_rank_expression(self, vectors, config):
        rank_expressions = [self.build_tsrank(vector, self.query, config=config) * boost for vector, boost in vectors]
