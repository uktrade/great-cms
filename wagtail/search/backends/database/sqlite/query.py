...
class MatchExpression(Expression):
    ...
    def as_sql(self, compiler, connection):
        ...
        params = [
            "{{{column}}} : ({query})".format(column=joined_columns, query=formatted_query)
...
def normalize(search_query: SearchQuery) -> Tuple[SearchQuery]:
    ...
    if isinstance(search_query, And):
        ...
        not_negated_subqueries = [
            subquery for subquery in normalized_subqueries if not isinstance(subquery, Not)
        ]  # All the non-negated subqueries.
        not_negated_subqueries = [
            subquery for subquery in not_negated_subqueries if not isinstance(subquery, MatchAll)
