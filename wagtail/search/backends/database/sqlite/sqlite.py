...
class Index:
    def __init__(self, backend, db_alias=None):
        ...
        if self.connection.vendor != "sqlite":
            raise NotSupportedError("You must select a SQLite database " "to use the SQLite search backend.")
...
class SQLiteSearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def build_search_query_content(self, query, config=None):
        ...
        if isinstance(query, PlainText):
            ...
            subquery_lexemes = [
                self.build_search_query_content(subquery, config=config) for subquery in query.subqueries
            ]
        ...
        raise NotImplementedError("`%s` is not supported by the SQLite search backend." % query.__class__.__name__)
    ...
    def search(self, config, start, stop, score_field=None):
        ...
        objs = (
            ...
            .filter(index_entry__content_type__in=get_descendants_content_types_pks(self.queryset.model))
        )
