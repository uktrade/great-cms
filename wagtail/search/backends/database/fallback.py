...
class DatabaseSearchQueryCompiler(BaseSearchQueryCompiler):
    ...
    def get_fields_names(self):
        ...
        fields_names = self.fields or [field.field_name for field in model.get_searchable_search_fields()]
    ...
    def build_database_filter(self, query, boost=1.0):
        if isinstance(query, PlainText):
            ...
            return operator([self.build_single_term_filter(term) for term in query.query_string.split()])
        ...
        if isinstance(query, And):
            subqueries = [self.build_database_filter(subquery, boost=boost) for subquery in query.subqueries]
        ...
        raise NotImplementedError("`%s` is not supported by the database search backend." % query.__class__.__name__)
