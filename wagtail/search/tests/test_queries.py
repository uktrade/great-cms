...
class TestGarbageCollectCommand(TestCase):
    def test_garbage_collect_command(self):
        ...
        self.assertFalse(
            models.QueryDailyHits.objects.filter(date=old_hit_date, query_id__in=querie_ids_to_be_deleted).exists()
        )

        self.assertEqual(models.Query.objects.filter(id__in=recent_querie_ids).count(), 10)
        self.assertEqual(
            models.QueryDailyHits.objects.filter(date=recent_hit_date, query_id__in=recent_querie_ids).count(),
            10,
        )

        self.assertEqual(models.Query.objects.filter(id__in=promoted_querie_ids).count(), 10)
        self.assertEqual(
            models.QueryDailyHits.objects.filter(date=recent_hit_date, query_id__in=promoted_querie_ids).count(),
            0,
        )
...
class TestParseQueryString(SimpleTestCase):
    ...
    def test_operator(self):
        ...
        self.assertEqual(
            repr(query),
            repr(Or([PlainText("this is simple", operator="or"), Phrase("hello world")])),
        )
