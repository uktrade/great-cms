...
def group_models_by_index(backend, models):
    ...
    return collections.OrderedDict(
        [(indices[index_name], index_models) for index_name, index_models in models_by_index.items()]
    )
    ...
    def update_backend(self, backend_name, schema_only=False, chunk_size=DEFAULT_CHUNK_SIZE):
        ...
        for index, models in models_grouped_by_index:
            ...
            if not schema_only:
                for model in models:
                    self.write(
                        "{}: {}.{} ".format(backend_name, model._meta.app_label, model.__name__).ljust(35),
                        ending="",
                    )

                    # Add items (chunk_size at a time)
                    for chunk in self.print_iter_progress(
                        self.queryset_chunks(model.get_indexed_objects().order_by("pk"), chunk_size)
                    ):
