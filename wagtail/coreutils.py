...
def get_locales_display_names() -> dict:
    ...
    locales_map = {locale.pk: locale.get_display_name() for locale in Locale.objects.all()}
    return locales_map
        ...
        # numpy < 1.9 and 1.9+ respectively
            ...
            try:  # attribute lookup
                ...
                try:  # list-index lookup
                    ...
                    raise AttributeError("Failed lookup for key [%s] in %r" % (bit, current))

        if callable(current):
            if getattr(current, "alters_data", False):
                raise SuspiciousOperation("Cannot call %r from multigetattr" % (current,))
