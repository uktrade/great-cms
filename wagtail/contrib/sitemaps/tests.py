...
class TestSitemapGenerator(TestCase):
    def setUp(self):
        ...
        self.other_site_homepage = root_page.add_child(
            instance=SimplePage(title="Another site", slug="another-site", content="bonjour", live=True)
        )
        Site.objects.create(hostname="other.example.com", port=80, root_page=self.other_site_homepage)
    ...
    def test_lastmod_uses_last_published_date(self):
        ...
        child_page_lastmod = [url["lastmod"] for url in urls if url["location"] == "http://localhost/hello-world/"][0]
        ...
        child_page_lastmod = [
            url["lastmod"] for url in urls if url["location"] == "http://localhost/no-last-publish-date/"
        ][0]
