...
def edit(request, redirect_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _("Redirect '%(redirect_title)s' updated.") % {"redirect_title": theredirect.title},
                buttons=[
                    messages.button(
...
def delete(request, redirect_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            _("Redirect '%(redirect_title)s' deleted.") % {"redirect_title": theredirect.title},
        )
        return redirect("wagtailredirects:index")
...
def add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _("Redirect '%(redirect_title)s' added.") % {"redirect_title": theredirect.title},
                buttons=[
                    messages.button(
                        reverse("wagtailredirects:edit", args=(theredirect.id,)),
                        _("Edit"),
...
def start_import(request):
    ...
    if extension not in supported_extensions:
        messages.error(
            request,
            _('File format of type "%(extension)s" is not supported') % {"extension": extension},
        )
    ...
    try:
        ...
        messages.error(
            request,
            _("Imported file has a wrong encoding: %(error_message)s") % {"error_message": e},
        )
        return redirect("wagtailredirects:start_import")
...
def process_import(request):
    ...
    input_format = get_import_formats()[int(management_form.cleaned_data["input_format"])]()
    ...
    messages.success(
        request,
        ngettext("Imported %(total)d redirect", "Imported %(total)d redirects", total) % {"total": total},
    )
