...
class ModelFormView(WMABaseView, FormView):
    ...
    def get_edit_handler(self):
        try:
            ...
            edit_handler = self.model_admin.get_edit_handler(instance=None, request=None)
            warnings.warn(
                "%s.get_edit_handler should not accept instance or request arguments" % type(self.model_admin).__name__,
    ...
    def get_context_data(self, form=None, **kwargs):
        ...
        bound_panel = self.edit_handler.get_bound_panel(form=form, instance=form.instance, request=self.request)
    ...
    def get_error_message(self):
        ...
        return _("The %(object)s could not be created due to errors.") % {"object": model_name}
...
class InstanceSpecificView(WMABaseView):
    ...
    def __init__(self, model_admin, instance_pk):
        ...
        object_qs = model_admin.model._default_manager.get_queryset().filter(**filter_kwargs)
        self.instance = get_object_or_404(object_qs)

        if getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(model_admin.model, TranslatableMixin):
...
class IndexView(SpreadsheetExportMixin, WMABaseView):
    ...
    def get_buttons_for_obj(self, obj):
        return self.button_helper.get_buttons_for_obj(obj, classnames_add=["button-small"])
...
class CreateView(ModelFormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.is_pagemodel:
            ...
            if parent_count == 1:
                ...
                return redirect(self.url_helper.get_action_url("add", self.app_label, self.model_name, parent_pk))
        ...
        if getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(self.model, TranslatableMixin):
    ...
    def get_form_kwargs(self):
        ...
        if getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(self.model, TranslatableMixin):
            selected_locale = self.request.GET.get("locale")
            if selected_locale:
                kwargs["instance"].locale = get_object_or_404(Locale, language_code=selected_locale)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(self.model, TranslatableMixin):
            ...
            for translation in self.instance.get_translations().select_related("locale"):
                ...
                url = self.url_helper.get_action_url("edit", translation.pk) + "?locale=" + locale.language_code
            ...
            for rel in fields:
                if rel.on_delete == models.PROTECT:
                    if isinstance(rel, OneToOneRel):
                        ...
                        for obj in qs.all():
                            linked_objects.append(obj)
            context = self.get_context_data(protected_error=True, linked_objects=linked_objects)
            return self.render_to_response(context)
...
class InspectView(InstanceSpecificView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if getattr(settings, "WAGTAIL_I18N_ENABLED", False) and issubclass(self.model_admin.model, TranslatableMixin):
            translations = []
            for translation in self.instance.get_translations().select_related("locale"):
                locale = translation.locale
                url = self.url_helper.get_action_url("inspect", translation.pk) + "?locale=" + locale.language_code
                translations.append({"locale": locale, "url": url})
    ...
    def get_context_data(self, **kwargs):
        context = {
            ...
            "buttons": self.button_helper.get_buttons_for_obj(self.instance, exclude=["inspect"]),
        }
        context.update(kwargs)
        return super().get_context_data(**context)
