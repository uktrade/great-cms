    ...
    def register_with_wagtail(self):
        ...
        if self.will_modify_explorer_page_queryset():
            ...
            def construct_explorer_page_queryset(parent_page, queryset, request):
                return self.modify_explorer_page_queryset(parent_page, queryset, request)
...
class ModelAdmin(WagtailRegisterable):
    ...
    def __init__(self, parent=None):
        ...
        self.parent = parent
        self.permission_helper = self.get_permission_helper_class()(self.model, self.inspect_view_enabled)
    ...
    def get_search_handler(self, request, search_fields=None):
        ...
        ``IndexView``.
        """
        return self.search_handler_class(search_fields or self.get_search_fields(request))
    ...
    def get_edit_handler(self, instance=None, request=None):
        ...
        if hasattr(self, "edit_handler"):
            ...
            try:
                ...
                warn(
                    "%s.get_form_fields_exclude should not accept a request argument" % type(self).__name__,
                    category=RemovedInWagtail50Warning,
                )
...
class ModelAdminGroup(WagtailRegisterable):
    ...
    def will_modify_explorer_page_queryset(self):
        return any(instance.will_modify_explorer_page_queryset() for instance in self.modeladmin_instances)

    def modify_explorer_page_queryset(self, parent_page, queryset, request):
        for instance in self.modeladmin_instances:
            queryset = instance.modify_explorer_page_queryset(parent_page, queryset, request)
        return queryset
