...
class TestTranslatableBookIndexView(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector_not_present_when_i18n_disabled(self):
        ...
        self.assertNotContains(response, '<a href="?locale__id__exact=2">French</a>', html=True)
...
class TestCreateView(TestCase, WagtailTestUtils):
    ...
    def test_create(self):
        ...
        self.client.post("/admin/modeladmintest/publisher/create/", {"name": "Sharper Collins"})
...
class TestRevisableCreateView(TestCase, WagtailTestUtils):
    ...
    def test_create_with_revision(self):
        ...
        log_entries = ModelLogEntry.objects.for_instance(instance).filter(action="wagtail.create")
...
class TestInspectView(TestCase, WagtailTestUtils):
    ...
    def test_book_extract_document_html_escaping(self):
        ...
        self.assertContains(response, "Title with &lt;script&gt;alert(&#x27;XSS&#x27;)&lt;/script&gt;")
...
class TestTranslatableBookEditView(TestCase, WagtailTestUtils):
    ...
    def get(self, book_id, **params):
        return self.client.get("/admin/modeladmintest/translatablebook/edit/%d/" % book_id, params)
...
class TestRevisableEditView(TestCase, WagtailTestUtils):
    ...
    def post(self, post_data):
        return self.client.post("/admin/modeladmintest/publisher/edit/%s/" % self.instance.pk, post_data)
    ...
    def test_edit_with_revision(self):
        ...
        # The log entry should have the revision attached
        log_entries = ModelLogEntry.objects.for_instance(instance).filter(action="wagtail.edit")
        self.assertEqual(log_entries.count(), 1)
        self.assertEqual(log_entries.first().revision, revision)
...
class TestDeleteViewWithProtectedRelation(TestCase, WagtailTestUtils):
    ...
    def test_post_with_dependent_object(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "'J. R. R. Tolkien' is currently referenced by other objects")
    ...
    def test_post_with_1to1_dependent_object(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "'Harper Lee' is currently referenced by other objects")
        self.assertContains(response, "<li><b>Solo Book:</b> To Kill a Mockingbird</li>")
...
class TestEditorAccess(TestCase, WagtailTestUtils):
    ...
    def test_index_permitted(self):
        ...
        self.group.permissions.add(Permission.objects.get(codename="add_book", content_type=self.book_content_type))
        response = self.client.get("/admin/modeladmintest/book/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_permitted(self):
        ...
        self.group.permissions.add(Permission.objects.get(codename="change_book", content_type=self.book_content_type))
        response = self.client.get("/admin/modeladmintest/book/edit/2/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_get_permitted(self):
        ...
        self.group.permissions.add(Permission.objects.get(codename="delete_book", content_type=self.book_content_type))
        response = self.client.get("/admin/modeladmintest/book/delete/2/")
        self.assertEqual(response.status_code, 200)
