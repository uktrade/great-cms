...
class TypedTableBlockValidationErrorAdapter(Adapter):
    ...
    def js_args(self, error):
        if error.cell_errors is None:
            ...
            return [
                {
                    row_index: {col_index: cell_error for col_index, cell_error in row_errors.items()}
...
class TypedTable:
    ...
    def rows(self):
        ...
        for row in self.row_data:
            yield [column["block"].bind(value) for column, value in zip(self.columns, row["values"])]
...
class BaseTypedTableBlock(Block):
    ...
    def value_from_datadict(self, data, files, prefix):
        ...
        rows = [
            {
                ...
                "values": [
                    col["block"].value_from_datadict(data, files, "%s-cell-%d-%d" % (prefix, row_index, col["id"]))
                    for col in columns
                ],
            }
        ...
        return TypedTable(
            columns=[{"block": col["block"], "heading": col["heading"]} for col in columns],
    ...
    def get_prep_value(self, table):
        if table:
            return {
                "columns": [{"type": col["block"].name, "heading": col["heading"]} for col in table.columns],
                "rows": [
                    {
                        "values": [
                            column["block"].get_prep_value(val) for column, val in zip(table.columns, row["values"])
                        ]
                    }
    ...
    def to_python(self, value):
        if value:
            ...
            columns_data = [
                col["block"].bulk_to_python([row["values"][column_index] for row in value["rows"]])
        ...
        if table:
            return {
                ...
                "rows": [
                    {
                        "values": [
                            column["block"].get_form_state(val) for column, val in zip(table.columns, row["values"])
                        ]
                    }
                    for row in table.row_data
                ],
