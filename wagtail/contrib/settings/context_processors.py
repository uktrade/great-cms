...
class SettingModuleProxy(dict):
    ...
    def get_setting(self, model_name):
        ...
        if Model is None:
            raise RuntimeError(f"Could not find model matching `{self.app_label}.{model_name}`.")
        ...
        raise NotImplementedError(
            "Setting models should inherit from either `BaseGenericSetting` " "or `BaseSiteSetting`."
        )
