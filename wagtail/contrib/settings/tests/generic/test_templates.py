...
class GenericSettingTemplateTestCase(GenericSettingsTestMixin, TestCase, WagtailTestUtils):
...
class GenericSettingTemplateTagTestCase(GenericSettingTemplateTestCase):
    ...
    def test_get_settings_request_context(self):
        ...
        template = Template(
            "{% load wagtailsettings_tags %}" "{% get_settings %}" "{{ settings.tests.testgenericsetting.title }}"
        )
    ...
    def test_models_cached(self):
        ...
        for i in range(1, 4):
            with self.assertNumQueries(1):
                ...
                self.assertEqual(template.render(context), self.default_settings.title * i)
