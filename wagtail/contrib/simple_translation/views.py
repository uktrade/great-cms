            ...
            with transaction.atomic():
                for locale in form.cleaned_data["locales"]:
                    if isinstance(self.object, Page):
                        ...
                        self.object.copy_for_translation(locale).save()  # pragma: no cover

                single_translated_object = None
                if len(form.cleaned_data["locales"]) == 1:
                    locales = form.cleaned_data["locales"][0].get_display_name()
                    single_translated_object = self.object.get_translation(form.cleaned_data["locales"][0])
                else:
                    # Translators: always plural
                    locales = _("%(locales_count)s locales") % {"locales_count": len(form.cleaned_data["locales"])}
...
class SubmitPageTranslationView(SubmitTranslationView):
    ...
    def get_success_message(self, locales):
        return _("The page '%(page_title)s' was successfully created in %(locales)s") % {
...
class SubmitSnippetTranslationView(SubmitTranslationView):
    def get_title(self):
        return _("Translate %(model_name)s") % {"model_name": self.object._meta.verbose_name}
