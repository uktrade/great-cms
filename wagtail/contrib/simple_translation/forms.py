    ...
    def __init__(self, instance, *args, **kwargs):
        ...
        if isinstance(instance, Page):
            ...
            if parent.is_root():
                ...
                allowed_locale_ids = (
                    instance.get_parent().get_translations(inclusive=True).values_list("locale_id", flat=True)
                )

            # Get and set the locale options that are disabled.
            disabled_locales = Locale.objects.exclude(id__in=allowed_locale_ids).values_list("id", flat=True)
            self.fields["locales"].widget.disabled_values = disabled_locales

            if disabled_locales:
                # Display a help text.
                url = reverse("simple_translation:submit_page_translation", args=[parent.id])
                help_text = ngettext(
            ...
            hide_select_all = len(untranslated_locales) == 1 or len(untranslated_locales) - len(disabled_locales) == 0
