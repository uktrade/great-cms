...
class FormBuilder:
    ...
    def get_formatted_field_initial(self, field):
        ...
        if "\n" in field.default_value:
            values = [x.strip().rstrip(",").strip() for x in field.default_value.split("\r\n")]
        else:
...
class WagtailAdminFormPageForm(WagtailAdminPageForm):
    def clean(self):
        ...
        if "form_fields" in self.formsets:
            ...
            clean_names = [f.instance.clean_name or f.instance.get_field_clean_name() for f in forms]
            duplicate_clean_name = next((n for n in clean_names if clean_names.count(n) > 1), None)
            if duplicate_clean_name:
                ...
                duplicate_form_field.add_error(
                    ...
                    django.forms.ValidationError(
                        _("There is another field with the label %(label_name)s, please change one of them.")
