...
class TestFormResponsesPanel(TestCase):
    def setUp(self):
        ...
        self.panel = panel.get_bound_panel(instance=self.form_page, form=self.FormPageForm(), request=self.request)
...
class TestFormsSubmissionsList(TestCase, WagtailTestUtils):
    ...
    def make_list_submissions(self):
        ...
        """
        for i in range(100):
            submission = FormSubmission(page=self.form_page, form_data={"hello": "world"})
            submission.save()
...
class TestDuplicateFormFieldLabels(TestCase, WagtailTestUtils):
    ...
    def test_adding_duplicate_form_labels(self):
        ...
        response = self.client.post(
            reverse("wagtailadmin_pages:add", args=("tests", "formpage", self.root_page.id)),
            post_data,
        )
    ...
    def test_rename_existing_field_and_add_new_field_with_clashing_clean_name(
        ...
        response = self.client.post(reverse("wagtailadmin_pages:edit", args=[form_page.pk]), post_data)
