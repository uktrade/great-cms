...
class TestGroupUsersView(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}, group_id=None):
        return self.client.get(
            reverse("wagtailusers_groups:users", args=(group_id or self.test_group.pk,)),
            params,
        )
...
class TestUserEditView(TestCase, WagtailTestUtils):
    ...
    def test_nonexistant_redirect(self):
        invalid_id = (
            "99999999-9999-9999-9999-999999999999" if settings.AUTH_USER_MODEL == "emailuser.EmailUser" else 100000
        )
...
class TestGroupEditView(TestCase, WagtailTestUtils):
    ...
    def test_group_edit_adding_document_permissions_same_collection(self):
        ...
        self.assertEqual(
            self.test_group.collection_permissions.filter(permission__content_type__app_label="wagtaildocs").count(),
            1,
        )
        response = self.post(
            {
    ...
    def test_group_edit_loads_with_page_permissions_shown(self):
        ...
        self.assertEqual(page_permissions_formset.forms[0]["permission_types"].value(), ["add"])

        # add edit permission on root
        GroupPagePermission.objects.create(page=self.root_page, permission_type="edit", group=self.test_group)
        ...
        GroupPagePermission.objects.create(page=self.home_page, permission_type="edit", group=self.test_group)
        ...
        self.assertEqual(page_permissions_formset.forms[1]["permission_types"].value(), ["edit"])
    ...
    def test_is_custom_permission_checked(self):
        ...
        self.assertTagInHTML(
            '<input type="checkbox" name="permissions" value="%s" checked>' % custom_permission.id,
            str(response.content),
        )
