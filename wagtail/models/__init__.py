...
from django.utils import timezone, translation as translation
...
def get_streamfield_names(model_class):
    return tuple(field.name for field in model_class._meta.concrete_fields if isinstance(field, StreamField))
...
class RevisionMixin(models.Model):
    ...
    def save_revision(
        ...
        logger.info('Edited: "%s" pk=%d revision_id=%d', str(self), self.pk, revision.id)
        if log_action:
...
class WorkflowMixin:
    ...
    def _check_draftstate_and_revision_mixins(cls):
        ...
        try:
            if not (mro.index(WorkflowMixin) < mro.index(DraftStateMixin) < mro.index(RevisionMixin)):
                return [error]
        except ValueError:
            return [error]
    ...
    def workflow_in_progress(self):
        ...
        return self.workflow_states.filter(status=WorkflowState.STATUS_IN_PROGRESS).exists()
    ...
    def current_workflow_state(self):
        ...
        return self.workflow_states.active().select_related("current_task_state__task").first()
...
class Page(AbstractPage, index.Indexed, ClusterableModel, metaclass=PageBase):
    ...
    def get_default_locale(self):
        ...
        if parent is not None:
            return parent.specific_class.objects.defer().select_related("locale").get(id=parent.id).locale
    ...
    def _update_descendant_url_paths(self, old_url_path, new_url_path):
        (
            ...
            .exclude(pk=self.pk)
            .update(url_path=Concat(Value(new_url_path), Substr("url_path", len(old_url_path) + 1)))
        )
    ...
    def update_aliases(self, *, revision=None, user=None, _content=None, _updated_ids=None):
        """
    ...
    def _get_relevant_site_root_paths(self, cache_object=None):
        ...
        """
        return tuple(srp for srp in self._get_site_root_paths(cache_object) if self.url_path.startswith(srp.root_path))
        ...
        if cls._clean_subpage_models is None:
            ...
            if subpage_types is None:
                ...
                cls._clean_subpage_models = [
                    resolve_model_string(model_string, cls._meta.app_label) for model_string in subpage_types
                ]
        ...
        if cls._clean_parent_page_models is None:
            ...
            if parent_page_types is None:
                ...
                cls._clean_parent_page_models = [
                    resolve_model_string(model_string, cls._meta.app_label) for model_string in parent_page_types
                ]
...
class Revision(models.Model):
    ...
    content = models.JSONField(verbose_name=_("content JSON"), encoder=DjangoJSONEncoder)
    ...
    def page(self):
        warnings.warn(
            "Revisions should access .content_object instead of .page " "to retrieve the object.",
            category=RemovedInWagtail50Warning,
            stacklevel=2,
        )
    ...
    def save(self, user=None, *args, **kwargs):
        ...
        if self.submitted_for_moderation:
            ...
            Revision.objects.filter(
                ...
                id=self.id
            ).update(submitted_for_moderation=False)
            ...
            log(
                ...
                data={
                    "revision": {
                        ...
                        "go_live_at": object.go_live_at.strftime("%d %b %Y %H:%M") if object.go_live_at else None,
    ...
    def as_page_object(self):
        warnings.warn(
            "Revisions should use .as_object() instead of .as_page_object() " "to create the object.",
            category=RemovedInWagtail50Warning,
            stacklevel=2,
        )
...
PAGE_PERMISSION_TYPE_CHOICES = [
    (identifier, long_label) for identifier, short_label, long_label in PAGE_PERMISSION_TYPES
]
...
class UserPagePermissionsProxy:
    ...
    def revisions_for_moderation(self):
        ...
        publishable_pages_paths = (
            self.permissions.filter(permission_type="publish").values_list("page__path", flat=True).distinct()
        )
...
class PagePermissionTester:
    def __init__(self, user_perms, page):
        ...
        if self.user.is_active and not self.user.is_superuser:
            self.permissions = {
                perm.permission_type for perm in user_perms.permissions if self.page.path.startswith(perm.page.path)
            }
...
class Task(models.Model):
    ...
    def locked_for_user(self, obj, user):
        ...
        if hasattr(self, "page_locked_for_user"):
            warnings.warn(
                "Tasks should use .locked_for_user() instead of " ".page_locked_for_user().",
...
class GroupApprovalTask(Task):
    groups = models.ManyToManyField(
        ...
        help_text=_("Pages/snippets at this step in a workflow will be moderated or approved by these groups of users"),
    )
    ...
    def start(self, workflow_state, user=None):
        if isinstance(workflow_state.content_object, LockableMixin) and workflow_state.content_object.locked_by:
            # If the person who locked the object isn't in one of the groups, unlock the object
            if not workflow_state.content_object.locked_by.groups.filter(id__in=self.groups.all()).exists():
                ...
                workflow_state.content_object.save(update_fields=["locked", "locked_by", "locked_at"])
...
class WorkflowState(models.Model):
    ...
    content_object = GenericForeignKey("base_content_type", "object_id", for_concrete_model=False)
    ...
    def clean(self):
        ...
        if self.status in (self.STATUS_IN_PROGRESS, self.STATUS_NEEDS_CHANGES):
            ...
            if (
                ...
                raise ValidationError(
                    _("There may only be one in progress or needs changes workflow state per page/snippet.")
                )
    ...
    def __str__(self):
        return _("Workflow '%(workflow_name)s' on %(model_name)s '%(title)s': %(status)s") % {
    ...
    def user_can_cancel(self, user):
        ...
        return (
            ...
            or (
                ...
                and self.current_task_state.status == self.current_task_state.STATUS_IN_PROGRESS
                and "approve"
                in [action[0] for action in self.current_task_state.task.get_actions(self.content_object, user)]
            )
        )
    ...
    def update(self, user=None, next_task=None):
        ...
        if current_status == TaskState.STATUS_REJECTED:
            ...
            if next_task:
                ...
                ) or self.current_task_state.status != self.current_task_state.STATUS_IN_PROGRESS:
    ...
    def successful_task_states(self):
        ...
        if getattr(settings, "WAGTAIL_WORKFLOW_REQUIRE_REAPPROVAL_ON_EDIT", False):
            successful_task_states = successful_task_states.filter(revision=self.content_object.get_latest_revision())
...
class TaskStateManager(models.Manager):
    ...
    def for_instance(self, instance):
        ...
        try:
            ...
            # Fallback to ContentType for the model
            return queryset.filter(
                workflow_state__content_type=ContentType.objects.get_for_model(instance, for_concrete_model=False),
                workflow_state__object_id=str(instance.pk),
            )
...
class TaskState(models.Model):
    ...
    def task_type_started_at(self):
        ...
        task_states = (
            TaskState.objects.filter(workflow_state=self.workflow_state).order_by("-started_at").select_related("task")
        )
        started_at = None
...
class PageLogEntryManager(BaseLogEntryManager):
    ...
    def viewable_by_user(self, user):
        ...
        if user.is_superuser or root_page_permissions.can_add_subpage() or root_page_permissions.can_edit():
            ...
            q = q | Q(page_id__in=Subquery(PageLogEntry.objects.filter(deleted=True).values("page_id")))
