...
class TranslatableMixin(models.Model):
    ...
    def get_default_locale(self):
        ...
        parental_keys = [
            ...
            if isinstance(field, ParentalKey) and issubclass(field.related_model, TranslatableMixin)
        ]

        if parental_keys:
            parent_id = parental_keys[0].value_from_object(self)
            return parental_keys[0].related_model.objects.defer().select_related("locale").get(id=parent_id).locale
...
class BootstrapTranslatableMixin(TranslatableMixin):
    ...
    locale = models.ForeignKey(Locale, on_delete=models.PROTECT, null=True, related_name="+", editable=False)
...
def get_translatable_models(include_subclasses=False):
    ...
    """
    translatable_models = [
        model for model in apps.get_models() if issubclass(model, TranslatableMixin) and not model._meta.abstract
    ]
    ...
    if include_subclasses is False:
        ...
        translatable_models = [model for model in translatable_models if model in root_translatable_models]
