...
class ChooserViewSet(ViewSet):
    ...
    choose_one_text = _("Choose")  #: Label for the 'choose' button in the chooser widget when choosing an initial item
    ...
    create_action_label = _("Create")  #: Label for the submit button on the 'create' form
    create_action_clicked_label = None  #: Alternative text to display on the submit button after it has been clicked
    creation_tab_label = (
        None  #: Label for the 'create' tab in the chooser modal (defaults to the same as create_action_label)
    )
    ...
    def on_register(self):
        if self.model and self.register_widget:
            register_form_field_override(ForeignKey, to=self.model, override={"widget": self.widget_class})
