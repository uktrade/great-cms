...
from wagtail.coreutils import (
    ...
    cautious_slugify as _cautious_slugify,
    escape_script,
...
def test_page_is_public(context, page):
    ...
    if not hasattr(context["request"], "all_page_view_restriction_paths"):
        context["request"].all_page_view_restriction_paths = PageViewRestriction.objects.select_related(
            "page"
...
def page_table_header_label(context, label=None, parent_page_title=None, **kwargs):
    ...
    if label:
        ...
        ascending_title_text = (
            _("Sort the order of child pages within '%(parent)s' by '%(label)s' in ascending order.")
            % translation_context
        )
        descending_title_text = (
            _("Sort the order of child pages within '%(parent)s' by '%(label)s' in descending order.")
            % translation_context
        )
    else:
...
@register.inclusion_tag("wagtailadmin/pages/listing/_modern_dropdown.html", takes_context=True)
...
def bulk_action_choices(context, app_label, model_name):
    ...
    bulk_action_buttons = [
        PageListingButton(
            action.display_name,
            reverse("wagtail_bulk_action", args=[app_label, model_name, action.action_type])
            + "?"
...
def wagtail_config(context):
    ...
    config = {
        ...
        "CSRF_HEADER_NAME": HttpHeaders.parse_header_name(getattr(settings, "CSRF_HEADER_NAME")),
        "ADMIN_URLS": {
