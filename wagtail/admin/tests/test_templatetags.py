...
class TestNotificationStaticTemplateTag(TestCase):
    ...
    def test_local_notification_static(self):
        ...
        self.assertEqual(
            "{}/static/wagtailadmin/images/email-header.jpg".format(settings.WAGTAILADMIN_BASE_URL),
            url,
        )

    @override_settings(STATIC_URL="/static/", WAGTAILADMIN_BASE_URL="http://localhost:8000")
...
class TestTimesinceTags(TestCase):
    ...
    def test_timesince_last_update_today_shows_time(self):
        ...
        timesince = timesince_last_update(dt, show_time_prefix=True, user_display_name="Gary")
    ...
    def test_timesince_last_update_before_today_shows_timeago(self):
        ...
        # Check user output
        timesince = timesince_last_update(dt, use_shorthand=False, user_display_name="Gary")
        self.assertEqual(timesince, "1\xa0week, 2\xa0days ago by Gary")
        self.assertEqual(
            timesince_last_update(dt, use_shorthand=False, user_display_name="Gary", show_time_prefix=True),
            timesince,
        )
    ...
    def test_human_readable_date(self):
        ...
        html = Template(template).render(Context({"date": now - timedelta(hours=1, minutes=10)}))
...
class TestComponentTag(TestCase):
    ...
    def test_component_escapes_unsafe_strings(self):
        ...
        template = Template("{% load wagtailadmin_tags %}<h1>{% component my_component %}</h1>")
        html = template.render(Context({"my_component": MyComponent()}))
