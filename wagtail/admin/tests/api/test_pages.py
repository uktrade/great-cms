...
class TestAdminPageListing(AdminAPITestCase, TestPageListing):
    ...
    def test_has_children_filter_off(self):
        ...
        page_id_list = self.get_page_id_list(content)
        self.assertEqual(page_id_list, [8, 9, 16, 18, 19, 10, 15, 17, 22, 23, 13, 14, 12, 25])
...
class TestCopyPageAction(AdminAPITestCase):
    ...
    def test_copy_page_with_exclude_fields_in_copy(self):
        ...
        self.assertEqual(new_page.special_field, new_page._meta.get_field("special_field").default)
    ...
    def test_without_publish_permissions_at_destination_with_keep_live(self):
        ...
        GroupPagePermission.objects.create(group=Group.objects.get(name="Editors"), page_id=2, permission_type="add")
    ...
    def test_copy_page_slug_in_use(self):
        ...
        self.assertEqual(
            content,
            {"slug": ["The slug 'events' is already in use within the parent page at '/'"]},
        )
...
class TestConvertAliasPageAction(AdminAPITestCase):
    ...
    def get_response(self, page_id):
        return self.client.post(reverse("wagtailadmin_api:pages:action", args=[page_id, "convert_alias"]))
...
class TestDeletePageAction(AdminAPITestCase):
    ...
    def get_response(self, page_id):
        return self.client.post(reverse("wagtailadmin_api:pages:action", args=[page_id, "delete"]))
...
class TestPublishPageAction(AdminAPITestCase):
    ...
    def get_response(self, page_id):
        return self.client.post(reverse("wagtailadmin_api:pages:action", args=[page_id, "publish"]))
    ...
    def test_publish_page(self):
        ...
        self.assertEqual(unpublished_page.first_published_at, unpublished_page.last_published_at)
...
class TestUnpublishPageAction(AdminAPITestCase):
    ...
    def get_response(self, page_id, data):
        return self.client.post(reverse("wagtailadmin_api:pages:action", args=[page_id, "unpublish"]), data)
...
class TestMovePageAction(AdminAPITestCase):
    ...
    def get_response(self, page_id, data):
        return self.client.post(reverse("wagtailadmin_api:pages:action", args=[page_id, "move"]), data)
...
class TestCopyForTranslationAction(AdminAPITestCase):
    ...
    def get_response(self, page_id, data):
        return self.client.post(
            reverse("wagtailadmin_api:pages:action", args=[page_id, "copy_for_translation"]),
            data,
        )
    ...
    def test_copy_childpage_with_copy_parents(self):
        response = self.get_response(self.en_eventindex.id, {"locale": "fr", "copy_parents": True})
        self.assertEqual(response.status_code, 201)
        content = json.loads(response.content.decode("utf-8"))
...
class TestCreatePageAliasAction(AdminAPITestCase):
    ...
    def test_create_alias(self):
        ...
        response = self.get_response(self.about_us.id, data={"update_slug": "new-about-us"})
    ...
    def test_create_alias_recursive(self):
        ...
        # Get christmas event
        old_christmas_event = self.events_index.get_children().filter(slug="christmas").first()
