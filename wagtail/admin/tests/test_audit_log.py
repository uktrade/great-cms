...
class TestAuditLogAdmin(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        for permission_type in ["add", "edit", "publish"]:
            GroupPagePermission.objects.create(group=sub_editors, page=self.hello_page, permission_type=permission_type)
    ...
    def test_history_with_deleted_user(self):
        ...
        # check page history
        response = self.client.get(reverse("wagtailadmin_pages:history", kwargs={"page_id": self.hello_page.id}))
    ...
    def test_edit_form_has_history_link(self):
        ...
        self.login(user=self.editor)
        response = self.client.get(reverse("wagtailadmin_pages:edit", args=[self.hello_page.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_and_publish_logs_revision_save(self):
        ...
        self.assertListEqual(
            list(PageLogEntry.objects.filter(page=page_id).values_list("action", flat=True).order_by("action")),
    ...
    def test_revert_and_publish_logs_reversion_and_publish(self):
        ...
        entries = PageLogEntry.objects.filter(page=self.hello_page).values_list("action", flat=True).order_by("action")
        self.assertListEqual(
            list(entries),
    ...
    def test_page_history_after_unscheduled_revision(self):
        ...
        history_url = reverse("wagtailadmin_pages:history", kwargs={"page_id": test_page.id})
        self.assertRedirects(
            response,
            history_url,
        )
