...
class TestGetFormForModel(TestCase):
    def test_get_form_without_model(self):
        ...
        with self.assertRaisesMessage(
            AttributeError,
            "ObjectList is not bound to a model yet. " "Use `.bind_to_model(model)` before using this method.",
        ):
...
class TestPageChooserPanel(TestCase):
    ...
    def test_render_js_init_with_can_choose_root_true(self):
        ...
        my_page_object_list = ObjectList([PageChooserPanel("page", can_choose_root=True)]).bind_to_model(
            PageChooserModel
        )
    ...
    def test_override_page_type(self):
        ...
        my_page_object_list = ObjectList([PageChooserPanel("page", "tests.EventPage")]).bind_to_model(
            EventPageChooserModel
        )
        my_page_chooser_panel = my_page_object_list.children[0]
        PageChooserForm = my_page_object_list.get_form_class()
...
class TestInlinePanel(TestCase, WagtailTestUtils):
    ...
    def test_invalid_inlinepanel_declaration(self):
        with self.ignore_deprecation_warnings():
            ...
            self.assertRaises(
                TypeError,
                lambda: InlinePanel(EventPage, "speakers", label="Speakers", bacon="chunky"),
            )
...
class TestInlinePanelGetComparison(TestCase):
    ...
    def test_get_comparison(self):
        ...
        comparison = page.get_edit_handler().bind_to(instance=page, request=self.request).get_comparison()
