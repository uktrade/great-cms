    ...
    def test_userbar_edit_menu_not_in_preview(self):
        ...
        content = template.render(
            Context(
                {
                    PAGE_TEMPLATE_VAR: self.homepage,
                    "request": self.dummy_request(self.user, is_preview=True, is_editing=True),
                }
            )
        )
        self.assertIn("<!-- Wagtail user bar embed code -->", content)
    ...
    def test_userbar_not_in_preview_panel(self):
        ...
        content = template.render(
            Context(
                {
                    PAGE_TEMPLATE_VAR: self.homepage,
                    "request": self.dummy_request(self.user, is_preview=True, in_preview_panel=True),
                }
            )
        )
...
class TestUserbarAddLink(TestCase, WagtailTestUtils):
    ...
    def test_page_disallowing_subpages(self):
        ...
        expected_link = '<a href="%s" target="_parent">Add a child page</a>' % expected_url
