...
class TestWorkflowsCreateView(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.snippet_content_type = ContentType.objects.get_for_model(FullFeaturedSnippet)
    ...
    def test_post(self):
        ...
        self.assertEqual(
            WorkflowTask.objects.get(workflow=workflow, task=self.task_1.task_ptr).sort_order,
            0,
        )
        self.assertEqual(
            WorkflowTask.objects.get(workflow=workflow, task=self.task_2.task_ptr).sort_order,
            1,
        )
    ...
    def test_snippet_already_has_workflow_check(self):
        workflow = Workflow.objects.create(name="existing_workflow")
        WorkflowContentType.objects.create(workflow=workflow, content_type=self.snippet_content_type)
...
class TestWorkflowsEditView(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        WorkflowContentType.objects.create(workflow=self.workflow, content_type=self.snippet_content_type)
    ...
    def test_snippet_already_has_workflow_check(self):
        ...
        WorkflowContentType.objects.filter(content_type=self.snippet_content_type).update(workflow=other_workflow)
...
class TestRemoveWorkflow(TestCase, WagtailTestUtils):
    ...
    def post(self, post_data={}):
        return self.client.post(
            reverse("wagtailadmin_workflows:remove", args=[self.page.id, self.workflow.id]),
            post_data,
        )
...
class TestSelectTaskTypeView(TestCase, WagtailTestUtils):
    ...
    def test_get_single_task_type(self):
        with mock.patch("wagtail.admin.views.workflows.get_task_types") as get_task_types:
...
class BasePageWorkflowTests(TestCase, WagtailTestUtils):
    ...
    def setup_workflow_and_tasks(self):
        ...
        WorkflowTask.objects.create(workflow=self.workflow, task=self.task_1, sort_order=1)
        WorkflowTask.objects.create(workflow=self.workflow, task=self.task_2, sort_order=2)
...
class BaseSnippetWorkflowTests(BasePageWorkflowTests):
    ...
    def setUp(self):
        ...
        self.moderator.user_permissions.add(self.edit_permission, self.publish_permission)
...
class TestSubmitPageToWorkflow(BasePageWorkflowTests):
    ...
    def test_submit_sends_mail(self):
        ...
        email_subject = (
            f'The {self.model_name} "{get_latest_str(self.object)}" has been submitted ' 'to workflow "test_workflow"'
        )
...
class TestApproveRejectPageWorkflow(BasePageWorkflowTests):
    ...
    def test_approve_task_and_workflow(self):
        ...
        workflow_state = WorkflowState.objects.for_instance(self.object).get(requested_by=self.submitter)
        ...
        # Object must be live
        self.assertTrue(self.object.live, msg="Approving moderation failed to set live=True")
    ...
    def test_approve_task_and_workflow_with_ajax(self):
        ...
        """
        # Post
        response = self.approve({"comment": "my comment"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
    ...
    def test_workflow_action_get(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailadmin/shared/workflow_action_modal.html")
    ...
    def test_workflow_action_view_not_in_moderation(self):
        ...
        self.assertContains(
            response,
            f"The {self.model_name} &#x27;{get_latest_str(self.object)}&#x27; " "is not currently awaiting moderation.",
        )
...
class TestApproveRejectSnippetWorkflow(TestApproveRejectPageWorkflow, BaseSnippetWorkflowTests):
...
class TestPageNotificationPreferences(BasePageWorkflowTests):
    ...
    def test_submitted_email_notifications_sent(self):
        ...
        account_notifications_url = get_admin_base_url() + reverse("wagtailadmin_account") + "#tab-notifications"
        self.assertIn(
    ...
    def test_approved_notifications(self):
        ...
        workflow_approved_emails = [
            email for email in mail.outbox if ("workflow" in email.subject and "approved" in email.subject)
        ]
    ...
    def test_rejected_notifications(self):
        ...
        workflow_rejected_emails = [
            email for email in mail.outbox if ("workflow" in email.subject and "rejected" in email.subject)
        ]
...
class TestSnippetNotificationPreferences(TestPageNotificationPreferences, BaseSnippetWorkflowTests):
    def setUp(self):
        super().setUp()
...
class TestDisableViews(BasePageWorkflowTests):
    def test_disable_workflow(self):
        ...
        states = WorkflowState.objects.for_instance(self.object).filter(workflow=self.workflow)
    ...
    def test_get_disable_workflow_shows_warning(self):
        ...
        response = self.client.get(reverse("wagtailadmin_workflows:disable", args=(self.workflow.pk,)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_disable_task(self):
        ...
        states = TaskState.objects.for_instance(self.object).filter(task=self.task_1.task_ptr)
...
class TestPageWorkflowPreview(BasePageWorkflowTests):
    ...
    def test_preview_workflow_show_edit_link_in_userbar(self):
        ...
        self.assertContains(response, reverse("wagtailadmin_pages:edit", args=(quote(self.object.pk),)))
...
class TestTaskChooserView(TestCase, WagtailTestUtils):
    ...
    def test_get_with_create_model_selected(self):
        response = self.client.get(
            reverse("wagtailadmin_workflows:task_chooser_create") + "?create_model=wagtailcore.GroupApprovalTask"
        )
    ...
    def test_get_with_non_task_create_model_selected(self):
        response = self.client.get(
            reverse("wagtailadmin_workflows:task_chooser_create") + "?create_model=wagtailcore.Page"
        )
    ...
    def test_get_with_base_task_create_model_selected(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_workflows:task_chooser_create") + "?create_model=wagtailcore.Task"
        )
