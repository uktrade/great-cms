...
def css_install_check(app_configs, **kwargs):
    ...
    css_path = os.path.join(os.path.dirname(__file__), "static", "wagtailadmin", "css", "core.css")
...
def base_form_class_check(app_configs, **kwargs):
    ...
    for cls in get_page_models():
        if not issubclass(cls.base_form_class, WagtailAdminPageForm):
            errors.append(
                Error(
                    "{}.base_form_class does not extend WagtailAdminPageForm".format(cls.__name__),
...
def check_panels_in_model(cls, context="model"):
    ...
    for panel_name in tabbed_panels:
        ...
        error_title = "{}.{} will have no effect on {} editing".format(class_name, panel_name, context)
...
def file_overwrite_check(app_configs, **kwargs):
    ...
    if file_storage == "storages.backends.s3boto3.S3Boto3Storage" and getattr(settings, "AWS_S3_FILE_OVERWRITE", True):
        errors.append(
            Warning(
    ...
    if file_storage == "storages.backends.gcloud.GoogleCloudStorage" and getattr(settings, "GS_FILE_OVERWRITE", True):
        errors.append(
            Warning(
