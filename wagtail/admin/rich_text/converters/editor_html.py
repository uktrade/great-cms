...
class DbWhitelister(Whitelister):
    ...
    * rewrites the attributes of any <a> element with a 'data-linktype' attribute, as
    ...
    def embed_handlers(self):
        return {rule.embed_type: rule.handler for rule in self.converter_rules if isinstance(rule, EmbedTypeRule)}

    @cached_property
    def link_handlers(self):
        return {rule.link_type: rule.handler for rule in self.converter_rules if isinstance(rule, LinkTypeRule)}
