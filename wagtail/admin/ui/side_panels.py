...
class BaseStatusSidePanel(BaseSidePanel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.show_schedule_publishing_toggle = show_schedule_publishing_toggle and not in_explorer
    ...
    def get_scheduled_publishing_context(self):
        ...
        if self.live_object and self.live_object.expire_at and not self.live_object.expired:
            ...
            if context["scheduled_go_live_at"] and context["scheduled_go_live_at"] < context["live_expire_at"]:
            ...
            context.update(
                {
                    ...
                    "history_url": reverse("wagtailadmin_pages:history", args=(page.id,)),
                    "workflow_history_url": reverse("wagtailadmin_pages:workflow_history", args=(page.id,)),
                    ...
                    "user_can_unlock": isinstance(self.lock, BasicLock) and user_perms.for_page(page).can_unlock(),
                    "locale": None,
                    "translations": [],
                }
            )
...
class PagePreviewSidePanel(BasePreviewSidePanel):
    def get_context_data(self, parent_context):
        context = super().get_context_data(parent_context)
        if self.object.id:
            context["preview_url"] = reverse("wagtailadmin_pages:preview_on_edit", args=[self.object.id])
        else:
