...
class CommentForm(WagtailAdminModelForm):
    ...
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            if (
                any(field for field in self.changed_data if field not in ["resolved", "position", "contentpath"])
                or cleaned_data["contentpath"].split(".")[0] != self.instance.contentpath.split(".")[0]
            ):
