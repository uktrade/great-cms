...
class TaskChooserSearchForm(forms.Form):
    q = forms.CharField(label=gettext_lazy("Search term"), widget=forms.TextInput(), required=False)
        ...
        if task_type_choices and len(task_type_choices) > 1:
            self.fields["task_type"] = forms.ChoiceField(
                choices=(
                    ...
                    + [(get_model_string(model), verbose_name) for model, verbose_name in task_type_choices]
                ),
                required=False,
            )

        # Save a mapping of task_type values back to the model that we can reference later
        self.task_type_choices = {get_model_string(model): model for model, verbose_name in task_type_choices}
        ...
        try:
            ...
            if not self.errors and existing_workflow != self.cleaned_data["workflow"]:
                ...
                self.add_error(
                    "page",
                    ValidationError(
                        _("This page already has workflow '%(workflow_name)s' assigned.")
...
class WorkflowContentTypeForm(forms.Form):
    ...
    class CheckboxSelectMultiple(forms.CheckboxSelectMultiple):
        ...
        option_template_name = "wagtailadmin/workflows/includes/workflow_content_types_checkbox.html"
        ...
        def render_with_errors(self, name, value, attrs=None, renderer=None, errors=None):
            context = {
    ...
    def __init__(self, *args, workflow=None, **kwargs):
        ...
        for model in get_workflow_enabled_models():
            workflow_enabled_q |= Q(app_label=model._meta.app_label, model=model._meta.model_name)

        self.fields["content_types"].queryset = ContentType.objects.filter(workflow_enabled_q)
    ...
    def clean(self):
        ...
        for assignment in existing_assignments:
            self.add_error(
                "content_types",
                ValidationError(
                    _("Snippet '%(content_type)s' already has workflow '%(workflow_name)s' assigned.")
                    % {
    ...
    def save(self, commit=True):
        ...
        with transaction.atomic():
            ...
            WorkflowContentType.objects.filter(workflow=self.workflow).exclude(content_type__in=content_types).delete()

            # Add any new content types, ignoring conflicts with existing ones
            # to avoid additional query for existing content types
            objects = [WorkflowContentType(workflow=self.workflow, content_type=ct) for ct in content_types]
        ...
        for field_name in getattr(task_model, "admin_form_readonly_on_edit_fields", []):
            if field_name not in form_class.base_fields:
                raise ImproperlyConfigured(
                    ...
                    "it to `%s.admin_form_fields`?" % (task_model.__name__, field_name, task_model.__name__)
                )
