...
class BaseCollectionMemberForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user is None:
            ...
            self.collections = self.permission_policy.collections_user_has_permission_for(user, "add")
        ...
        if len(self.collections) == 0:
            raise Exception("Cannot construct %s for a user with no collection permissions" % type(self))
...
class BaseGroupCollectionMemberPermissionFormSet(forms.BaseFormSet):
    ...
    def __init__(self, data=None, files=None, instance=None, prefix=None):
        ...
        if instance.pk is None:
            ...
            full_collection_permissions = (
                instance.collection_permissions.filter(permission__in=self.permission_queryset)
    ...
    def save(self):
        if self.instance.pk is None:
            raise Exception("Cannot save a GroupCollectionMemberPermissionFormSet " "for an unsaved group instance")

        # get a set of (collection, permission) tuples for all ticked permissions
        forms_to_save = [
            form for form in self.forms if form not in self.deleted_forms and "collection" in form.cleaned_data
        ]
        ...
        GroupCollectionPermission.objects.bulk_create(
            [
                GroupCollectionPermission(group=self.instance, collection=collection, permission=permission)
