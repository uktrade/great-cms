...
def get_requested_by_queryset(request):
    User = get_user_model()
    return User.objects.filter(pk__in=set(WorkflowState.objects.values_list("requested_by__pk", flat=True))).order_by(
        User.USERNAME_FIELD
    )
...
def get_editable_content_type_ids(request):
    ...
    return [ct.id for ct in ContentType.objects.get_for_models(*editable_models).values()]
...
class WorkflowReportFilterSet(WagtailFilterSet):
    ...
    requested_by = django_filters.ModelChoiceFilter(field_name="requested_by", queryset=get_requested_by_queryset)
    ...
    def filter_reviewable(self, queryset, name, value):
        if value and self.request and self.request.user:
            queryset = queryset.filter(
                id__in=TaskState.objects.reviewable_by(self.request.user).values_list("id", flat=True)
            )
        return queryset
...
class WorkflowView(ReportView):
    ...
    def get_filename(self):
        return "workflow-report-{}".format(datetime.datetime.today().strftime("%Y-%m-%d"))
    ...
    def get_queryset(self):
        ...
        editable_objects = Q(content_type_id__in=get_editable_content_type_ids(self.request))

        return WorkflowState.objects.filter(editable_pages | editable_objects).order_by("-created_at")
...
class WorkflowTasksView(ReportView):
    ...
    def get_filename(self):
        return "workflow-tasks-{}".format(datetime.datetime.today().strftime("%Y-%m-%d"))
    ...
    def get_queryset(self):
        ...
        editable_objects = Q(workflow_state__content_type_id__in=get_editable_content_type_ids(self.request))
        return TaskState.objects.filter(editable_pages | editable_objects).order_by("-started_at")
