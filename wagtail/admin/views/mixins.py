...
class SpreadsheetExportMixin:
    ...
    custom_value_preprocess = {
        datetime.datetime: {
            FORMAT_XLSX: lambda value: (value if timezone.is_naive(value) else timezone.make_naive(value, timezone.utc))
        },
    ...
    def to_row_dict(self, item):
        ...
        row_dict = OrderedDict((field, multigetattr(item, field)) for field in self.list_export)
        return row_dict
    ...
    def generate_xlsx_row(self, worksheet, row_dict, date_format=None):
        ...
        for field, value in row_dict.items():
            cell = WriteOnlyCell(worksheet, self.preprocess_field_value(field, value, self.FORMAT_XLSX))
    ...
    def write_csv_row(self, writer, row_dict):
        return writer.writerow(
            {field: self.preprocess_field_value(field, value, self.FORMAT_CSV) for field, value in row_dict.items()}
        )
    ...
    def write_xlsx(self, queryset, output):
        ...
        worksheet.append(self.get_heading(queryset, field) for field in self.list_export)

        date_format = ExcelDateFormatter().get()
        for item in queryset:
            worksheet.append(self.generate_xlsx_row(worksheet, self.to_row_dict(item), date_format=date_format))
