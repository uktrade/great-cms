...
class PageTitleColumn(Column):
    ...
    def __init__(self, *args, show_locale_labels=False, is_multiple_choice=False, **kwargs):
...
class PageNavigateToChildrenColumn(Column):
    cell_template_name = "wagtailadmin/chooser/tables/page_navigate_to_children_cell.html"
...
class BrowseView(View):
    ...
    def columns(self):
        ...
        if self.is_multiple_choice:
            cols.insert(
                0,
                PageCheckboxSelectColumn("select", label=_("Select"), width="1%", accessor="pk"),
            )
        return cols
    ...
    def get(self, request, parent_page_id=None):
        ...
        target_pages = Page.objects.filter(pk__in=[int(pk) for pk in request.GET.getlist("target_pages[]", []) if pk])
        ...
        if self.i18n_enabled:
            if self.parent_page.is_root():
                ...
                locale_options = [
                    ...
                    for locale in Locale.objects.filter(pk__in=pages.values_list("locale_id")).exclude(
                        pk=active_locale_id
                    )
                ]
            else:
                ...
                locales_and_parent_pages = {
                    item["locale"]: item["pk"]
                    for item in Page.objects.translation_of(self.parent_page).values("locale", "pk")
                }
                locales_and_parent_pages[selected_locale.pk] = self.parent_page.pk
                for locale in Locale.objects.filter(pk__in=list(locales_and_parent_pages.keys())).exclude(
        ...
        context = shared_context(
            request,
            {
                ...
                "page_type_names": [desired_class.get_verbose_name() for desired_class in self.desired_classes],
...
class SearchView(View):
    ...
    def columns(self):
        cols = [
            PageTitleColumn("title", label=_("Title"), show_locale_labels=self.i18n_enabled),
            ParentPageColumn("parent", label=_("Parent"), show_locale_labels=self.i18n_enabled),
            DateColumn(
                "updated",
                label=_("Updated"),
                width="12%",
...
class BaseLinkFormView(View):
    ...
    def get(self, request):
        self.form = self.form_class(initial=self.get_initial_data(), prefix=self.form_prefix)
        return self.render_form_response()

    def post(self, request):
        self.form = self.form_class(request.POST, initial=self.get_initial_data(), prefix=self.form_prefix)
