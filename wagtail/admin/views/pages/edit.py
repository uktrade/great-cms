...
class EditView(TemplateResponseMixin, ContextMixin, HookResponseMixin, View):
    ...
    def add_save_confirmation_message(self):
        if self.is_reverting:
            message = _(
                ...
                "previous_revision_datetime": self.previous_revision.created_at.strftime("%d %b %Y %H:%M"),
            }
        else:
            message = _("Page '%(page_title)s' has been updated.") % {"page_title": self.page.get_admin_display_title()}
    ...
    def dispatch(self, request, page_id):
        ...
        self.locked_for_user = self.lock is not None and self.lock.for_user(self.request.user)
        ...
        if getattr(settings, "WAGTAIL_WORKFLOW_ENABLED", True):
            ...
            self.workflow_state = (
                self.page.current_workflow_state or self.page.workflow_states.order_by("created_at").last()
            )
        else:
    ...
    def get(self, request):
        if self.lock:
            ...
            if lock_message:
                ...
                if isinstance(self.lock, ScheduledForPublishLock) and self.page_perms.can_unschedule():
                    lock_message = format_html(
                        '{} <span class="buttons"><button type="button" class="button button-small button-secondary" data-action="w-action#post" data-controller="w-action" data-w-action-url-value="{}">{}</button></span>',
                        lock_message,
                        reverse(
    ...
    def publish_action(self):
        ...
        if go_live_at and go_live_at > timezone.now():
            ...
            if self.is_reverting:
                ...
                if self.page.live:
                    message = _("Page '%(page_title)s' is live and this version has been scheduled for publishing.") % {
                        "page_title": self.page.get_admin_display_title()
                    }

                else:
                    message = _("Page '%(page_title)s' has been scheduled for publishing.") % {
                        "page_title": self.page.get_admin_display_title()
                    }
            ...
            if self.is_reverting:
                message = _("Version from %(datetime)s of page '%(page_title)s' has been published.") % {
                    "datetime": self.previous_revision.created_at.strftime("%d %b %Y %H:%M"),
                    "page_title": self.page.get_admin_display_title(),
                }
            else:
    ...
    def form_invalid(self, form):
        ...
        self.errors_debug = repr(self.form.errors) + repr(
            [(name, formset.errors) for (name, formset) in self.form.formsets.items() if formset.errors]
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        bound_panel = self.edit_handler.get_bound_panel(instance=self.page, request=self.request, form=self.form)
        ...
        context.update(
            {
                ...
                "locale": None,
                "translations": [],
                "media": bound_panel.media + self.form.media + action_menu.media + side_panels.media,
            }
        )
