...
class PagesForModerationPanel(Component):
    ...
    def get_context_data(self, parent_context):
        ...
        context["page_revisions_for_moderation"] = (
            user_perms.revisions_for_moderation().select_related("user").order_by("-created_at")
        )
...
class UserObjectsInWorkflowModerationPanel(Component):
    ...
    def get_context_data(self, parent_context):
        ...
        if getattr(settings, "WAGTAIL_WORKFLOW_ENABLED", True):
            ...
            pages_owned_by_user = Q(base_content_type_id=get_default_page_content_type().id) & Exists(
                Page.objects.filter(
                    owner=request.user,
...
def icons():
    ...
    if _icons_html is None:
        ...
        for icon in all_icons:
            combined_icon_markup += (
                render_to_string(icon).replace('xmlns="http://www.w3.org/2000/svg"', "").replace("svg", "symbol")
            )
