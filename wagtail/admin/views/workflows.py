...
class Index(IndexView):
    ...
    def get_queryset(self):
        ...
        content_types = WorkflowContentType.objects.filter(workflow=OuterRef("pk")).values_list("pk", flat=True)
...
class Edit(EditView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["content_type_form"] = self.get_content_type_form()
        context["can_disable"] = (
            self.permission_policy is None or self.permission_policy.user_has_permission(self.request.user, "delete")
        ) and self.object.active
        context["can_enable"] = (
            self.permission_policy is None or self.permission_policy.user_has_permission(self.request.user, "create")
...
def enable_workflow(request, pk):
    ...
    if not workflow.active:
        ...
        messages.success(
            request,
            _("Workflow '%(workflow_name)s' enabled.") % {"workflow_name": workflow.name},
        )
...
def remove_workflow(request, page_pk, workflow_pk=None):
    ...
    if hasattr(page, "workflowpage"):
        ...
        if not workflow_pk or workflow_pk == page.workflowpage.workflow.pk:
            ...
            messages.success(
                request,
                _("Workflow removed from Page '%(page_title)s'.") % {"page_title": page.get_admin_display_title()},
            )
...
def enable_task(request, pk):
    ...
    if not task.active:
        task.active = True
        task.save()
        messages.success(request, _("Task '%(task_name)s' enabled.") % {"task_name": task.name})
...
class BaseTaskChooserView(TemplateView):
    def dispatch(self, request):
        ...
        self.can_create = task_permission_policy.user_has_permission(request.user, "add") and len(self.task_models) != 0
    ...
    def get_task_type_filter_choices(self):
        ...
        """
        task_type_choices = [(model, model.get_verbose_name()) for model in self.task_models]
    ...
    def get_create_tab_context_data(self):
        return {
            ...
            "add_url": reverse("wagtailadmin_workflows:task_chooser_create") + "?" + self.request.GET.urlencode()
            if self.create_model
            else None,
...
class TaskChooserCreateView(BaseTaskChooserView):
    ...
    def render_to_response(self, context):
        ...
        return render_modal_workflow(self.request, None, None, None, json_data=js_context)
