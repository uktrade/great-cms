...
from django.views.generic.edit import (
    BaseCreateView,
    BaseDeleteView as DjangoBaseDeleteView,
    BaseUpdateView,
    DeletionMixin,
    FormMixin,
)
...
class IndexView(LocaleMixin, PermissionCheckedMixin, WagtailAdminTemplateMixin, BaseListView):
    model = None
    index_url_name = None
    add_url_name = None
    ...
    def get_search_form(self):
        ...
        if self.is_searchable and self.search_kwarg in self.request.GET:
            return SearchForm(
                self.request.GET,
                placeholder=_("Search %(model_name)s") % {"model_name": self.model._meta.verbose_name_plural},
            )

        return SearchForm(placeholder=_("Search %(model_name)s") % {"model_name": self.model._meta.verbose_name_plural})
    ...
    def get_queryset(self):
        ...
        if not queryset.ordered:
            queryset = queryset.order_by(models.F("_updated_at").desc(nulls_last=True), "-pk")
    ...
    def search_queryset(self, queryset):
        ...
        if class_is_indexed(queryset.model):
            ...
            return search_backend.search(self.search_query, queryset, fields=self.search_fields)

        filters = {field + "__icontains": self.search_query for field in self.search_fields or []}
    ...
    def _get_title_column(self, field_name, column_class=TitleColumn, **kwargs):
        ...
        return self._get_custom_column(field_name, column_class, get_url=self.get_edit_url, **kwargs)
    ...
    def get_context_data(self, *args, object_list=None, **kwargs):
        ...
        context["can_add"] = self.permission_policy is None or self.permission_policy.user_has_permission(
        ...
        if self.filters:
            ...
            context["is_filtering"] = any(self.request.GET.get(f) for f in set(self.filters.get_fields()))
...
class EditView(
    ...
    def get_success_buttons(self):
        return [messages.button(reverse(self.edit_url_name, args=(quote(self.object.pk),)), _("Edit"))]
    ...
    def get_context_data(self, **kwargs):
        ...
        context["submit_button_label"] = self.submit_button_label
        context["can_delete"] = self.permission_policy is None or self.permission_policy.user_has_permission(
...
class RevisionsCompareView(WagtailAdminTemplateMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        revision_a, revision_a_heading = self._get_revision_and_heading(self.revision_id_a)
        revision_b, revision_b_heading = self._get_revision_and_heading(self.revision_id_b)
...
class RevisionsUnscheduleView(TemplateView):
    ...
    success_message = gettext_lazy('Version %(revision_id)s of "%(object)s" unscheduled.')
