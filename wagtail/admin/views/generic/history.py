...
class WorkflowHistoryDetailView(BaseObjectMixin, TemplateView):
    ...
    def workflow_state(self):
        return get_object_or_404(
            WorkflowState.objects.for_instance(self.object).filter(id=self.kwargs[self.workflow_state_url_kwarg]),
        )
    ...
    def revisions(self):
        ...
        return (
            ...
            .filter(
                id__in=TaskState.objects.filter(workflow_state=self.workflow_state).values_list(
        ...
        task_states_by_revision_task = [
            (
                revision,
                {
                    ...
                    for task_state in TaskState.objects.filter(workflow_state=self.workflow_state, revision=revision)
                },
            )
