...
class PreviewOnEdit(View):
    ...
    def remove_old_preview_data(self):
        ...
        expired_keys = [
            k for k, v in self.request.session.items() if k.startswith(self.session_key_prefix) and v[1] < expiration
        ]
...
class PreviewRevision(View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            preview_mode = request.GET.get("mode", self.revision_object.default_preview_mode)
        except IndexError:
            raise PermissionDenied
