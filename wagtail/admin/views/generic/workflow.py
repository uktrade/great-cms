...
class WorkflowAction(BaseWorkflowFormView):
    ...
    def post(self, request, *args, **kwargs):
        if self.form_class:
            ...
            elif self.action_modal and self.request.headers.get("x-requested-with") == "XMLHttpRequest":
                # show form errors
                return self.render_modal_form(self.request, form)
        else:
            self.redirect_url = (
                self.task.on_action(self.task_state, self.request.user, self.action_name) or self.redirect_url
            )
...
class ConfirmWorkflowCancellation(BaseObjectMixin, View):
    ...
    def dispatch(self, request, *args, **kwargs):
        if not self.workflow_state or not getattr(settings, "WAGTAIL_WORKFLOW_CANCEL_ON_PUBLISH", True):
            return render_modal_workflow(
                request,
                "",
                None,
    ...
    def get_context_data(self, **kwargs):
        return {
            "needs_changes": self.workflow_state.status == WorkflowState.STATUS_NEEDS_CHANGES,
...
class PreviewRevisionForTask(BaseObjectMixin, View):
    ...
    def add_error_message(self):
        messages.error(
            self.request,
            _("The %(model_name)s '%(title)s' is not currently awaiting moderation in task '%(task_name)s'.")
            % {
                "model_name": self.model._meta.verbose_name,
                "title": get_latest_str(self.object),
