...
class BaseChooseView(
    ...
    def get_chosen_multiple_url(self):
        return self.append_preserved_url_parameters(reverse(self.chosen_multiple_url_name))
    ...
    def checkbox_column(self):
        return CheckboxSelectColumn("select", label=_("Select"), width="1%", accessor="pk")
...
class CreationFormMixin(ModelLookupMixin, PreserveURLParametersMixin):
    ...
    def can_create(self):
        return self.get_permission_policy().user_has_permission(self.request.user, "add")
    ...
    def get_create_url(self):
        if not self.create_url_name:
            raise ImproperlyConfigured(
                "%r must provide a create_url_name attribute or a get_create_url method" % type(self)
            )
        ...
        context.update(
            {
                ...
                "creation_tab_label": self.creation_tab_label or self.create_action_label,
            }
        )
...
class CreateViewMixin:
    ...
    def get_reshow_creation_form_response(self):
        ...
        response_html = render_to_string(self.creation_form_template_name, context, self.request)
        return render_modal_workflow(
            self.request,
            None,
            None,
