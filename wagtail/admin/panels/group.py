        ...
        for child in self.children:
            ...
            for key, new_val in child_options.items():
                if key not in options:
                    ...
                    if isinstance(new_val, list) or isinstance(new_val, dict) or isinstance(new_val, set):
                        options[key] = new_val.copy()
                    else:
                        options[key] = new_val
                else:
                    current_val = options[key]
                    if isinstance(current_val, list) and isinstance(new_val, (list, tuple)):
                        current_val.extend(new_val)
                    elif isinstance(current_val, tuple) and isinstance(new_val, (list, tuple)):
                        ...
                        raise ValueError(
                            "Don't know how to merge values %r and %r for form option %r" % (current_val, new_val, key)
                        )
    ...
    class BoundPanel(Panel.BoundPanel):
        ...
        def children(self):
            return [
                ...
                for child, identifier in zip(self.panel.children, self.panel.child_identifiers)
            ]
        ...
        def visible_children_with_identifiers(self):
            return [
                (child, identifier)
                for child, identifier in zip(self.children, self.panel.child_identifiers)
                if child.is_shown()
            ]
