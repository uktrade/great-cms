...
class Panel:
    ...
    def get_bound_panel(self, instance=None, request=None, form=None, prefix="panel"):
        ...
        if self.model is None:
            raise ImproperlyConfigured(
                "%s.bind_to_model(model) must be called before get_bound_panel" % type(self).__name__
            )

        if not issubclass(self.BoundPanel, Panel.BoundPanel):
            raise ImproperlyConfigured("%s.BoundPanel must be a subclass of Panel.BoundPanel" % type(self).__name__)

        return self.BoundPanel(panel=self, instance=instance, request=request, form=form, prefix=prefix)
    ...
    class BoundPanel(Component):
        ...
        def render_missing_fields(self):
            ...
            missing_fields_html = [
                str(self.form[field_name]) for field_name in self.form.fields if field_name not in rendered_fields
            ]
