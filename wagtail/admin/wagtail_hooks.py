...
class CollectionsMenuItem(MenuItem):
    def is_shown(self, request):
        return collection_permission_policy.user_has_any_permission(request.user, ["add", "change", "delete"])
...
def page_listing_more_buttons(page, page_perms, next_url=None):
    ...
    if page_perms.can_reorder_children():
        yield Button(
            ...
            "?ordering=ord",
            attrs={
                "title": _("Change ordering of child pages of '%(title)s'") % {"title": page.get_admin_display_title()}
            },
            priority=60,
        )
...
def page_header_buttons(page, page_perms, next_url=None):
    if page_perms.can_edit():
        yield Button(
            ...
            reverse("wagtailadmin_pages:edit", args=[page.id]),
            icon_name="edit",
            attrs={"title": _("Edit '%(title)s'") % {"title": page.get_admin_display_title()}},
            priority=10,
        )
    if page_perms.can_move():
        yield Button(
...
def register_core_features(features):
    ...
    for order, element in enumerate(headings_elements):
        features.register_converter_rule("editorhtml", element, [WhitelistRule(element, allow_without_attributes)])
    ...
    # Draftail
    features.register_editor_plugin("draftail", "hr", draftail_features.BooleanFeature("enableHorizontalRule"))
    features.register_converter_rule(
        "contentstate",
        "hr",
        {
            ...
            "to_database_format": {"entity_decorators": {"HORIZONTAL_RULE": lambda props: DOM.create_element("hr")}},
        },
    )
    ...
    features.register_converter_rule(
        ...
        "ul",
        {
            ...
            "to_database_format": {"block_map": {"unordered-list-item": {"element": "li", "wrapper": "ul"}}},
        },
    )
    features.register_editor_plugin(
        "draftail",
    ...
    features.register_converter_rule(
        ...
        "ol",
        {
            ...
            "to_database_format": {"block_map": {"ordered-list-item": {"element": "li", "wrapper": "ol"}}},
        },
    )
    features.register_editor_plugin(
        "draftail",
