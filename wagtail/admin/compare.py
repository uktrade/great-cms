...
def register_comparison_class(field_class, to=None, comparison_class=None, exact_class=False):
    """
    ...
    if comparison_class is None:
        raise ImproperlyConfigured("register_comparison_class must be passed a 'comparison_class' keyword argument")

    if to and field_class != models.ForeignKey:
        raise ImproperlyConfigured("The 'to' argument on register_comparison_class is only valid for ForeignKey fields")

    comparison_class_registry.register(field_class, to=to, value=comparison_class, exact_class=exact_class)
...
class RichTextFieldComparison(TextFieldComparison):
    def htmldiff(self):
        return diff_text(text_from_html(self.val_a), text_from_html(self.val_b)).to_html()
    ...
    def htmlvalue(self, val):
        ...
        for name, block in self.block.child_blocks.items():
            ...
            htmlvalues.append(
                (
                    label,
                    comparison_class(block, True, True, val[name], val[name]).htmlvalue(val[name]),
                )
            )
    ...
    def get_block_comparisons(self):
        ...
        if a_length > b_length:
            for block in a_blocks[b_length:]:
                # Deleted blocks
                comparisons.append(comparison_class(block.block, True, False, block.value, None))
