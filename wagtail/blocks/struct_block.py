...
class StructBlockValidationErrorAdapter(Adapter):
    ...
    def js_args(self, error):
        if error.block_errors is None:
            ...
            return [{name: error_list.as_data() for name, error_list in error.block_errors.items()}]
...
class BaseStructBlock(Block):
    ...
    def bulk_to_python(self, values):
        ...
        return [
            self._to_struct_value({name: values_by_subfield[name][i] for name in self.child_blocks.keys()})
            for i in range(0, len(values))
        ]
    ...
    def get_api_representation(self, value, context=None):
        ...
        return {
            name: self.child_blocks[name].get_api_representation(val, context=context) for name, val in value.items()
        }
