...
class StreamBlockValidationErrorAdapter(Adapter):
    ...
    def js_args(self, error):
        return [
            ...
            {block_id: child_errors.as_data() for block_id, child_errors in error.block_errors.items()},
        ]
...
class BaseStreamBlock(Block):
    ...
    def clean(self, value):
        ...
        if self.meta.min_num is not None and self.meta.min_num > len(value):
            non_block_errors.append(
                ValidationError(_("The minimum number of items is %(min_num)d") % {"min_num": self.meta.min_num})
            )
        ...
        if self.meta.max_num is not None and self.meta.max_num < len(value):
            non_block_errors.append(
                ValidationError(_("The maximum number of items is %(max_num)d") % {"max_num": self.meta.max_num})
            )
                ...
                if min_num is not None and min_num > block_count:
                    non_block_errors.append(
                        ValidationError(
                            "{}: {}".format(
                                block.label,
                                _("The minimum number of items is %(min_num)d") % {"min_num": min_num},
                            )
                        )
                    )
                if max_num is not None and max_num < block_count:
                    non_block_errors.append(
                        ValidationError(
                            "{}: {}".format(
                                block.label,
                                _("The maximum number of items is %(max_num)d") % {"max_num": max_num},
                            )
                        )
                    )
    ...
    def bulk_to_python(self, values):
        ...
        return [
            StreamValue(
                self,
                [(block_type, child_outputs[block_type][child_index], id) for block_type, id, child_index in block_map],
                is_lazy=False,
            )
...
class StreamValue(MutableSequence):
    ...
    class BlockNameLookup(Mapping):
        ...
        def __init__(self, stream_value, find_all=True):
            ...
            self.find_all = find_all  # whether to return all results rather than just the first
