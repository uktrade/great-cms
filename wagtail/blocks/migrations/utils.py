...
class InvalidBlockDefError(Exception):
    ...
    def __str__(self):
        ...
        if self.instance is not None:
            message += "Invalid block def in {} object ({})".format(self.instance.__class__.__name__, self.instance.id)
    ...
    for child_block in stream_block_value:
        if not should_alter_block(child_block["type"], block_path):
            ...
            try:
                ...
                raise InvalidBlockDefError("No current block def named {}".format(child_block["type"]))
...
def apply_changes_to_raw_data(raw_data, block_path_str, operation, streamfield, **kwargs):
    """
