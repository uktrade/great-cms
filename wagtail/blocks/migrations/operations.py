...
class RemoveStreamChildrenOperation(BaseBlockOperation):
    ...
    def apply(self, block_value):
        return [child_block for child_block in block_value if child_block["type"] != self.name]
...
class RemoveStructChildrenOperation(BaseBlockOperation):
    ...
    def apply(self, block_value):
        return {child_key: child_value for child_key, child_value in block_value.items() if child_key != self.name}
...
class ListChildrenToStructBlockOperation(BaseBlockOperation):
    ...
    def apply(self, block_value):
        ...
        for child_block in formatted_list_child_generator(block_value):
            mapped_block_value.append({**child_block, "value": {self.block_name: child_block["value"]}})
        return mapped_block_value
