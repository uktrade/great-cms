...
class EventPage(Page):
    ...
    promote_panels = [
        MultiFieldPanel(COMMON_PANELS, "Common page configuration", help_text="For SEO nerds only"),
...
class HeadCountRelatedModelUsingPK(models.Model):
    ...
    event_page = ParentalKey(EventPage, on_delete=models.CASCADE, related_name="head_counts")
...
class ExtendedFormField(AbstractFormField):
    ...
    field_type = models.CharField(verbose_name="field type", max_length=16, choices=EXTENDED_CHOICES)
...
class AdvertWithTabbedInterface(models.Model):
    ...
    edit_handler = TabbedInterface(
        [
            ...
            ObjectList(other_panels, heading="Other", help_text="Other panels help text"),
        ],
...
class SingletonPage(Page):
    ...
    def can_create_at(cls, parent):
        ...
        return super(SingletonPage, cls).can_create_at(parent) and not cls.objects.exists()
...
class ImportantPagesSiteSetting(BaseSiteSetting):
    sign_up_page = models.ForeignKey("wagtailcore.Page", related_name="+", null=True, on_delete=models.SET_NULL)
    general_terms_page = models.ForeignKey("wagtailcore.Page", related_name="+", null=True, on_delete=models.SET_NULL)
    privacy_policy_page = models.ForeignKey("wagtailcore.Page", related_name="+", null=True, on_delete=models.SET_NULL)
...
class GenericSnippetPage(Page):
    ...
    snippet_content_type = models.ForeignKey(ContentType, on_delete=models.SET_NULL, null=True, blank=True)
...
class Address(index.Indexed, ClusterableModel, Orderable):
    ...
    person = ParentalKey(to="tests.PersonPage", related_name="addresses", verbose_name="Person")
...
class AddressTag(TaggedItemBase):
    content_object = ParentalKey(to="tests.Address", on_delete=models.CASCADE, related_name="tagged_items")
...
class TaggedRestaurant(ItemBase):
    tag = models.ForeignKey(RestaurantTag, related_name="tagged_restaurants", on_delete=models.CASCADE)
    content_object = ParentalKey(to="tests.RestaurantPage", on_delete=models.CASCADE, related_name="tagged_items")
...
class TaggedRestaurantDocument(ItemBase):
    tag = models.ForeignKey(RestaurantTag, related_name="tagged_documents", on_delete=models.CASCADE)
...
class TaggedRestaurantImage(ItemBase):
    tag = models.ForeignKey(RestaurantTag, related_name="tagged_images", on_delete=models.CASCADE)
    content_object = models.ForeignKey(
...
class CustomRestaurantImage(AbstractImage):
    tags = TaggableManager(help_text=None, blank=True, verbose_name="tags", through=TaggedRestaurantImage)
    admin_form_fields = Image.admin_form_fields
...
class GalleryPage(Page):
    content_panels = Page.content_panels + [MultipleChooserPanel("gallery_images", chooser_field_name="image")]


class GalleryPageImage(Orderable):
    page = ParentalKey("tests.GalleryPage", related_name="gallery_images", on_delete=models.CASCADE)
    image = models.ForeignKey(
        "wagtailimages.Image",
        on_delete=models.CASCADE,
        related_name="+",
