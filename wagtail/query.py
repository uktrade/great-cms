...
class PageQuerySet(SearchableQuerySetMixin, TreeQuerySet):
    ...
    def type_q(self, *types):
        all_subclasses = {model for model in apps.get_models() if issubclass(model, types)}
    ...
    def annotate_approved_schedule(self):
        ...
        return self.annotate(
            _approved_schedule=Exists(
                Revision.page_revisions.exclude(approved_go_live_at__isnull=True).filter(
    ...
    def annotate_site_root_state(self):
        ...
        """
        return self.annotate(
            _is_site_root=Exists(Site.objects.filter(root_page__translation_key=OuterRef("translation_key")))
        )
            ...
            if missing_pks:
                ...
                warnings.warn(
                    ...
                    "the relevant table or record since the page was created:\n{}".format(
                        [{"id": p.id, "title": p.title, "type": p.content_type} for p in generic_pages.values()]
                    ),
