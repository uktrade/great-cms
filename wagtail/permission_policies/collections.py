...
class CollectionPermissionPolicy(CollectionPermissionLookupMixin, BaseDjangoAuthPermissionPolicy):
    ...
    def instances_user_has_any_permission_for(self, user, actions):
        ...
        if not (user.is_active and user.is_authenticated):
            ...
            return self.model.objects.filter(collection__in=list(self._collections_with_perm(user, actions)))
...
class CollectionOwnershipPermissionPolicy(CollectionPermissionLookupMixin, BaseDjangoAuthPermissionPolicy):
    ...
    def instances_user_has_any_permission_for(self, user, actions):
        if user.is_active and user.is_superuser:
            ...
            add_perm_filter = Q(collection__in=list(self._collections_with_perm(user, ["add"]))) & Q(
            ...
            choose_perm_filter = Q(collection__in=list(self._collections_with_perm(user, ["choose"])))
    ...
    def users_with_any_permission_for_instance(self, actions, instance):
        if "change" in actions or "delete" in actions:
            ...
            if owner is not None and self._check_perm(owner, ["add"], collection=instance.collection):
            ...
            # over this instance's collection.
            filter_expr = self._users_with_perm_filter(["choose"], collection=instance.collection)
            return get_user_model().objects.filter(filter_expr).distinct()
        else:
            # action is either not recognised, or is the 'add' action which is
...
class CollectionMangementPermissionPolicy(CollectionPermissionLookupMixin, BaseDjangoAuthPermissionPolicy):
    def _descendants_with_perm(self, user, action):
        ...
        if collection_roots:
            ...
            collection_path_filter = Q(path__startswith=collection_roots[0]["path"]) & Q(
                depth__gt=collection_roots[0]["depth"]
            )
