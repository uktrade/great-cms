...
class AdminSnippetChooser(BaseChooser):
    ...
    def get_chooser_modal_url(self):
        try:
            return reverse(f"wagtailsnippetchoosers_{self.model._meta.app_label}_{self.model._meta.model_name}:choose")
            ...
            if self.model not in get_snippet_models():
                raise ImproperlyConfigured("AdminSnippetChooser cannot be used on non-snippet model %r" % self.model)
            else:
                raise
