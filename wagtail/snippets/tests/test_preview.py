...
class TestPreview(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.preview_on_add_url = reverse("wagtailsnippets_tests_previewablemodel:preview_on_add")
    ...
    def test_preview_on_edit_with_valid_then_invalid_data(self):
        ...
        response = self.client.post(self.preview_on_edit_url, {**self.post_data, "text": ""})
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
            response.content.decode(),
    ...
    def test_preview_on_edit_expiry(self):
        ...
        expiry_datetime = initial_datetime + datetime.timedelta(seconds=PreviewOnEdit.preview_expiration_timeout + 1)
...
class TestEnablePreview(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.multiple = MultiPreviewModesModel.objects.create(text="Multiple preview modes")
    ...
    def test_show_preview_panel_on_create_with_multiple_modes(self):
        ...
        # Should respect the default_preview_mode
        self.assertContains(response, '<option value="alt#1" selected>Alternate</option>')

    def test_show_preview_panel_on_edit_with_single_mode(self):
        edit_url = self.get_url(self.single, "edit", args=(self.single.pk,))
        preview_url = self.get_url(self.single, "preview_on_edit", args=(self.multiple.pk,))
        iframe_url = preview_url + "?in_preview_panel=true&mode="
    ...
    def test_show_preview_panel_on_edit_with_multiple_modes(self):
        ...
        preview_url = self.get_url(self.multiple, "preview_on_edit", args=(self.multiple.pk,))
        iframe_url = preview_url + "?in_preview_panel=true&mode=alt%231"
        response = self.client.get(edit_url)
...
class TestDisablePreviewWithEmptyModes(TestCase, WagtailTestUtils):
    ...
    def test_disable_preview_on_revisions_list(self):
        ...
        preview_url = self.get_url("revisions_view", args=(self.snippet.pk, latest_revision.id))
...
class TestDisablePreviewWithoutMixin(TestDisablePreviewWithEmptyModes):
    ...
    def get_url(self, name, args=None):
        ...
        if name == "revisions_view":
            return f"/admin/snippets/tests/{self.model_name}/history/" f"{args[0]}/revisions/{args[1]}/view/"
