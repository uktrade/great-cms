...
class TestLocaleSelectorOnList(TestCase, WagtailTestUtils):
    ...
    def test_locale_selector_not_present_on_non_translatable_snippet(self):
        ...
        self.assertNotContains(response, 'aria-label="French" class="u-link is-live w-no-underline">')
...
class TestSnippetCreateView(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}, model=Advert):
        ...
        return self.client.get(reverse(f"wagtailsnippets_{app_label}_{model_name}:add"), params)

    def post(self, post_data={}, model=Advert):
        app_label = model._meta.app_label
        model_name = model._meta.model_name
        return self.client.post(reverse(f"wagtailsnippets_{app_label}_{model_name}:add"), post_data)
...
class TestEditDraftStateSnippet(BaseTestSnippetEditView):
    ...
    def test_publish(self):
        ...
        log_entries = ModelLogEntry.objects.filter(
            content_type=ContentType.objects.get_for_model(DraftStateCustomPrimaryKeyModel),
    ...
    def test_edit_post_scheduled(self):
        ...
        self.assertFalse(
            Revision.objects.for_instance(self.test_snippet).exclude(approved_go_live_at__isnull=True).exists()
        )
...
class TestSnippetHistory(TestCase, WagtailTestUtils):
    ...
    def test_should_show_actions_on_revisable_snippet(self):
        ...
        self.assertNotContains(response, '<span class="status-tag primary">Live version</span>')
        self.assertNotContains(response, '<span class="status-tag primary">Current draft</span>')
    ...
    def test_with_live_and_draft_status(self):
        ...
        self.assertContains(response, '<span class="status-tag primary">Live version</span>', count=1)

        # Should show the "current draft" status tag for the draft revision
        self.assertContains(response, '<span class="status-tag primary">Current draft</span>', count=1)
...
class TestSnippetChooseStatus(TestCase, WagtailTestUtils):
    ...
    def test_choose_results_view_shows_status_column(self):
        ...
        self.assertContains(response, '<div class="status-tag primary">live</div>', html=True)
        self.assertContains(response, '<div class="status-tag primary">live + draft</div>', html=True)
...
class TestSnippetListViewWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("wagtailsnippets_snippetstests_standardsnippetwithcustomprimarykey:list"),
            params,
        )
...
class TestSnippetViewWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def get(self, snippet, params={}):
        ...
        model_name = snippet._meta.model_name
        args = [quote(snippet.pk)]
        return self.client.get(reverse(f"wagtailsnippets_{app_label}_{model_name}:edit", args=args), params)
...
class TestSnippetChooserPanelWithCustomPrimaryKey(TestCase, WagtailTestUtils):
    ...
    def test_target_model_autodetected(self):
        edit_handler = ObjectList([FieldPanel("advertwithcustomprimarykey")]).bind_to_model(
            SnippetChooserModelWithCustomPrimaryKey
        )
        form_class = edit_handler.get_form_class()
        form = form_class()
