...
def get_edit_url(app_label, model_name, snippet):
    return reverse(f"wagtailsnippets_{app_label}_{model_name}:edit", args=[quote(snippet.pk)])
...
class SnippetBulkAction(BulkAction):
    ...
    def object_context(self, snippet):
        return {
            "item": snippet,
            "edit_url": get_edit_url(self.model._meta.app_label, self.model._meta.model_name, snippet),
        }
