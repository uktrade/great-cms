...
def get_editable_models(user):
    ...
    return [model for model in SNIPPET_MODELS if user.has_perm(get_permission_name("change", model))]
...
def _register_snippet_immediately(model, viewset=None):
    ...
    model.get_usage = lambda obj: ReferenceIndex.get_references_to(obj).group_by_source_object()
    ...
    register_form_field_override(ForeignKey, to=model, override={"widget": AdminSnippetChooser(model=model)})
...
def create_extra_permissions(*args, using=DEFAULT_DB_ALIAS, **kwargs):
    ...
    model_cts = ContentType.objects.get_for_models(*SNIPPET_MODELS, for_concrete_models=False)
