...
from wagtail.admin.views.generic.preview import (
    PreviewOnCreate as PreviewOnCreateView,
    PreviewOnEdit as PreviewOnEditView,
    PreviewRevision,
)
        ...
        context.update(
            {
                ...
                "can_add_snippet": self.permission_policy.user_has_permission(self.request.user, "add"),
                "can_delete_snippets": self.permission_policy.user_has_permission(self.request.user, "delete"),
            }
        )
...
class CreateView(generic.CreateEditViewOptionalFeaturesMixin, generic.CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        side_panels = SnippetSidePanels(
            ...
            self.model(),
            self,
            show_schedule_publishing_toggle=getattr(form, "show_schedule_publishing_toggle", False),
        )
        ...
        if self.locale:
            context["translations"] = [
                {
                    "locale": locale,
                    "url": reverse(self.add_url_name) + "?locale=" + locale.language_code,
                }
                for locale in Locale.objects.all().exclude(id=self.locale.id)
            ]
...
class EditView(generic.CreateEditViewOptionalFeaturesMixin, generic.EditView):
    ...
    def get_context_data(self, **kwargs):
        ...
        side_panels = SnippetSidePanels(
            ...
            scheduled_object=self.live_object.get_scheduled_revision_as_object() if self.draftstate_enabled else None,
        )
        media = context.get("media") + action_menu.media + side_panels.media
    ...
    def get_context_data(self, **kwargs):
        ...
        for object, references in context.get("page_obj"):
            ...
            if edit_url is None:
                ...
                label = str(object)
                edit_link_title = _("Edit this %(object)s") % {"object": object._meta.verbose_name}
...
def redirect_to_edit(request, app_label, model_name, pk):
    return redirect(f"wagtailsnippets_{app_label}_{model_name}:edit", pk, permanent=True)


def redirect_to_delete(request, app_label, model_name, pk):
    return redirect(f"wagtailsnippets_{app_label}_{model_name}:delete", pk, permanent=True)


def redirect_to_usage(request, app_label, model_name, pk):
    return redirect(f"wagtailsnippets_{app_label}_{model_name}:usage", pk, permanent=True)
...
class RevisionsCompareView(PermissionCheckedMixin, generic.RevisionsCompareView):
    ...
    @property
    def edit_label(self):
        return _("Edit this %(model_name)s") % {"model_name": self.model._meta.verbose_name}

    @property
    def history_label(self):
        return _("%(model_name)s history") % {"model_name": self.model._meta.verbose_name}
...
class LockView(PermissionCheckedMixin, lock.LockView):
    ...
    def user_has_permission(self, permission):
        ...
        if permission == self.permission_required and isinstance(self.object, WorkflowMixin):
            current_workflow_task = self.object.current_workflow_task
            if current_workflow_task:
                return current_workflow_task.user_can_lock(self.object, self.request.user)
    ...
    def user_has_permission(self, permission):
        ...
        if permission == self.permission_required:
            ...
            if isinstance(self.object, WorkflowMixin):
                current_workflow_task = self.object.current_workflow_task
                if current_workflow_task:
                    return current_workflow_task.user_can_unlock(self.object, self.request.user)
...
class SnippetViewSet(ViewSet):
    ...
    def edit_view(self):
        ...
        return self.edit_view_class.as_view(
            ...
            confirm_workflow_cancellation_url_name=self.get_url_name("confirm_workflow_cancellation"),
        )
    ...
    def workflow_history_view(self):
        return self.workflow_history_view_class.as_view(
            ...
            permission_policy=self.permission_policy,
            workflow_history_url_name=self.get_url_name("workflow_history"),
            workflow_history_detail_url_name=self.get_url_name("workflow_history_detail"),
        )
    ...
    def on_register(self):
        super().on_register()
        url_finder_class = type("_SnippetAdminURLFinder", (SnippetAdminURLFinder,), {"model": self.model})
