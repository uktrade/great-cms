...
class TestPublishScheduledPagesCommand(WagtailTestUtils, TestCase):
    ...
    def test_go_live_page_will_be_published(self):
        ...
        self.assertTrue(
            Revision.page_revisions.filter(object_id=p.id).exclude(approved_go_live_at__isnull=True).exists()
        )
    ...
    def test_expired_pages_are_dropped_from_mod_queue(self):
        ...
        p = Page.objects.get(slug="hello-world")
        self.assertFalse(p.live)
        self.assertTrue(Revision.page_revisions.filter(object_id=p.id, submitted_for_moderation=True).exists())

        management.call_command("publish_scheduled_pages")

        p = Page.objects.get(slug="hello-world")
        self.assertFalse(Revision.page_revisions.filter(object_id=p.id, submitted_for_moderation=True).exists())
...
class TestPurgeRevisionsCommand(TestCase):
    ...
    def test_latest_revision_not_purged(self):
        ...
        self.assertNotIn(revision_1, Revision.page_revisions.filter(object_id=self.page.id))
        self.assertIn(revision_2, Revision.page_revisions.filter(object_id=self.page.id))
    ...
    def test_revisions_in_moderation_not_purged(self):
        ...
        self.assertTrue(Revision.page_revisions.filter(object_id=self.page.id, submitted_for_moderation=True).exists())
    ...
    def test_revisions_with_approve_go_live_not_purged(self):
        ...
        self.assertIn(approved_revision, Revision.page_revisions.filter(object_id=self.page.id))
    ...
    def test_purge_revisions_with_date_cutoff(self):
        ...
        self.assertIn(old_revision, Revision.page_revisions.filter(object_id=self.page.id))
        ...
        self.assertNotIn(old_revision, Revision.page_revisions.filter(object_id=self.page.id))
