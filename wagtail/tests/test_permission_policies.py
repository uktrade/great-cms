            ...
            for action, expected_result in expected_results:
                if expected_result:
                    self.assertTrue(
                        ...
                        "User %s should be able to %s instance %s, but can't" % (user, action, instance),
                    )
                else:
                    self.assertFalse(
                        self.policy.user_has_permission_for_instance(user, action, instance),
                        "User %s should not be able to %s instance %s, but can" % (user, action, instance),
                    )
...
class TestBlanketPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_user_has_any_permission_for_instance(self):
        for user in self.all_users:
            self.assertTrue(
                self.policy.user_has_any_permission_for_instance(user, ["change", "delete"], self.adder_image)
            )
...
class TestAuthenticationOnlyPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_user_has_any_permission_for_instance(self):
        ...
        self.assertTrue(
            self.policy.user_has_any_permission_for_instance(self.superuser, ["change", "delete"], self.adder_image)
        )
        ...
        self.assertTrue(
            self.policy.user_has_any_permission_for_instance(self.useless_user, ["change", "delete"], self.adder_image)
        )
    ...
    def test_instances_user_has_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.anonymous_user, "change"),
            no_images,
        )
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        # the set of images editable by superuser includes all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.superuser, ["change", "delete"]),
            all_images,
        )

        # the set of images editable by inactive superuser includes no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.inactive_superuser, ["change", "delete"]),
            no_images,
        )

        # the set of images editable by ordinary user includes all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.useless_user, ["change", "delete"]),
            all_images,
        )

        # the set of images editable by anonymous user includes no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.anonymous_user, ["change", "delete"]),
            no_images,
        )
...
class TestModelPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_user_has_any_permission_for_instance(self):
        ...
        # in order for user_has_any_permission to return true
        self.assertTrue(
            self.policy.user_has_any_permission_for_instance(self.image_changer, ["change", "delete"], self.adder_image)
        )
        self.assertTrue(
            self.policy.user_has_any_permission_for_instance(
        ...
        # should return false
        self.assertFalse(
            self.policy.user_has_any_permission_for_instance(self.image_adder, ["change", "delete"], self.adder_image)
        )
    ...
    def test_instances_user_has_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.oneoff_image_changer, "change"),
            all_images,
        )
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_permission_for(self.inactive_image_changer, "change"),
            no_images,
        )
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        # given the relevant model permission at the group level, a user can edit all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.image_changer, ["change", "delete"]),
            all_images,
        )

        # given the relevant model permission at the user level, a user can edit all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.oneoff_image_changer, ["change", "delete"]),
            all_images,
        )
        ...
        # an inactive user with the relevant permission can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.inactive_image_changer, ["change", "delete"]),
            no_images,
        )

        # a user with permission, but not the matching one, can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.image_adder, ["change", "delete"]),
            no_images,
        )
...
class TestOwnershipPermissionPolicy(PermissionPolicyTestCase):
    ...
    def test_instances_user_has_any_permission_for(self):
        ...
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.image_adder, ["delete", "frobnicate"]),
            [self.adder_image],
        )

        # a user with 'edit' permission can change/delete all images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.oneoff_image_changer, ["delete", "frobnicate"]),
            all_images,
        )
        ...
        # a user with permission, but not the matching one, can edit no images
        self.assertResultSetEqual(
            self.policy.instances_user_has_any_permission_for(self.oneoff_image_deleter, ["change", "delete"]),
            no_images,
        )
