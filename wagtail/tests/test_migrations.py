...
class TestForMigrations(TestCase):
    def test__migrations(self):
        app_labels = {app.label for app in apps.get_app_configs() if app.name.startswith("wagtail.")}
        ...
        if changes:
            migrations = "\n".join(
                (
                    "  {migration}\n{changes}".format(
                        migration=migration,
                        changes="\n".join("    {0}".format(operation.describe()) for operation in migration.operations),
                    )
