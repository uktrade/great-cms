...
class TestStreamValueAccess(TestCase):
    ...
    def setUp(self):
        self.json_body = self.model.objects.create(body=json.dumps([{"type": "text", "value": "foo"}]))
...
class TestRequiredStreamField(TestCase):
    ...
    def test_non_blank_field_is_required(self):
        ...
        field = StreamField(MyStreamBlock(), blank=False, use_json_field=self.use_json_field)
        self.assertTrue(field.stream_block.required)
        with self.assertRaises(StreamBlockValidationError):
            field.stream_block.clean([])
    ...
    def test_blank_false_is_implied_by_default(self):
        # passing a block list
        field = StreamField([("paragraph", blocks.CharBlock())], use_json_field=self.use_json_field)
        self.assertTrue(field.stream_block.required)
        with self.assertRaises(StreamBlockValidationError):
            field.stream_block.clean([])
        ...
        field = StreamField(MyStreamBlock(required=False), use_json_field=self.use_json_field)
        self.assertTrue(field.stream_block.required)
        with self.assertRaises(StreamBlockValidationError):
            field.stream_block.clean([])
    ...
    def test_blank_field_is_not_required(self):
        ...
        # passing a block instance
        field = StreamField(MyStreamBlock(), blank=True, use_json_field=self.use_json_field)
        self.assertFalse(field.stream_block.required)
        field.stream_block.clean([])  # no validation error on empty stream

        field = StreamField(MyStreamBlock(required=True), blank=True, use_json_field=self.use_json_field)
        self.assertFalse(field.stream_block.required)
        field.stream_block.clean([])  # no validation error on empty stream
...
class TestStreamFieldCountValidation(TestCase):
    ...
    def test_minimum_count(self):
        ...
        self.assertEqual(catcher.exception.params, {"__all__": ["The minimum number of items is 2"]})
    ...
    def test_maximum_count(self):
        ...
        with self.assertRaises(StreamBlockValidationError) as catcher:
            instance.body.stream_block.clean(instance.body)
        self.assertEqual(catcher.exception.params, {"__all__": ["The maximum number of items is 5"]})
