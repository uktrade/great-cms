...
from wagtail.test.testapp.blocks import LinkBlock as CustomLinkBlock, SectionBlock
...
class TestDecimalBlock(TestCase):
    ...
    def test_type_to_python(self):
        block = blocks.DecimalBlock()
        block_val = block.to_python("1.63")  # decimals get saved as string in JSON field
        self.assertEqual(type(block_val), Decimal)
    ...
    def test_round_trip_to_db_preserves_type(self):
        ...
        db_value = json.dumps(block.get_prep_value(original_value), cls=DjangoJSONEncoder)
...
class TestRegexBlock(TestCase):
    ...
    def test_raises_custom_error_message(self):
        ...
        block = blocks.RegexBlock(regex=r"^[0-9]{3}$", error_messages={"invalid": test_message})
...
class TestRichTextBlock(TestCase):
    ...
    def test_validate_max_length(self):
        ...
        block.clean(RichText('<p><a href="http://really-long-domain-name.example.com">also</a> short</p>'))
...
class TestMultipleChoiceBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_adapt_multiple_choice_block(self):
        ...
        self.assertEqual(js_args[0], "test_choiceblock")
        self.assertIsInstance(js_args[1], forms.Select)
        self.assertEqual(list(js_args[1].choices), [("tea", "Tea"), ("coffee", "Coffee")])
        self.assertEqual(
            js_args[2],
            {
                "label": "Test choiceblock",
...
class TestStructBlock(SimpleTestCase):
    ...
    def test_initialisation_with_mixins(self):
        ...
        self.assertEqual(list(block.child_blocks.keys()), ["title", "link", "classname", "source"])
    ...
    def test_get_api_representation_calls_same_method_on_fields_with_context(self):
        ...
        self.assertDictEqual(api_representation, {"language": "English", "author": "Wagtail!"})
    ...
    def test_adapt_with_help_text_on_meta(self):
        ...
        self.assertEqual(
            js_args[2],
            {
                ...
                "classname": "struct-block",
                "helpIcon": (
                    '<svg class="icon icon-help default" aria-hidden="true">' '<use href="#icon-help"></use></svg>'
                ),
...
class TestListBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_render_calls_block_render_on_children(self):
        ...
        """
        block = blocks.ListBlock(blocks.CharBlock(template="tests/blocks/heading_block.html"))
...
class TestStreamBlock(WagtailTestUtils, SimpleTestCase):
    ...
    def test_adapt(self):
        ...
        grouped_blocks = [(group_name, list(group_iter)) for (group_name, group_iter) in js_args[1]]
    ...
    def test_min_num_validation_errors(self):
        ...
        with self.assertRaises(ValidationError) as catcher:
            block.clean(value)
        self.assertEqual(catcher.exception.params, {"__all__": ["The minimum number of items is 1"]})
    ...
    def test_max_num_validation_errors(self):
        ...
        with self.assertRaises(ValidationError) as catcher:
            block.clean(value)
        self.assertEqual(catcher.exception.params, {"__all__": ["The maximum number of items is 1"]})
    ...
    def check_get_prep_value_nested_streamblocks(self, stream_data, is_lazy):
        ...
        value = {k: blocks.StreamValue(block.child_blocks[k], v, is_lazy=is_lazy) for k, v in stream_data.items()}
        jsonish_value = block.get_prep_value(value)
    ...
    def test_block_names(self):
        ...
        result = {block_name: [block.value for block in blocks] for block_name, blocks in blocks_by_name.items()}
        self.assertEqual(
            result,
            {
                "heading": ["My title"],
...
class TestStructBlockWithFixtures(TestCase):
    ...
    def test_extract_references(self):
        ...
        self.assertListEqual(
            list(block.extract_references({"page": christmas_page, "link_text": "Christmas"})),
            [
...
class TestPageChooserBlock(TestCase):
    ...
    def test_deconstruct_target_model_default(self):
        block = blocks.PageChooserBlock()
        self.assertEqual(block.deconstruct(), ("wagtail.blocks.PageChooserBlock", (), {}))
