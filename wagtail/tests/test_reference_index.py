...
class TestCreateOrUpdateForObject(TestCase):
    def setUp(self):
        ...
        self.event_page.carousel_items = [
            EventPageCarouselItem(caption="1234567", image=self.test_image_1, sort_order=1),
            EventPageCarouselItem(caption="7654321", image=self.test_image_2, sort_order=2),
            EventPageCarouselItem(caption="abcdefg", image=self.test_image_1, sort_order=3),
        ]
    ...
    def test_null_parental_key(self):
        obj = ModelWithNullableParentalKey(
            content="""<p><a linktype="page" id="%d">event page</a></p>""" % self.event_page.id
        )
        obj.save()
