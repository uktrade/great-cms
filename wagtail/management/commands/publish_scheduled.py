...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        expired_revs = [r for r in Revision.objects.filter(submitted_for_moderation=True) if revision_date_expired(r)]
        if dryrun:
            self.stdout.write("\n---------------------------------")
            if expired_revs:
        ...
        revs_for_publishing = Revision.objects.filter(approved_go_live_at__lt=timezone.now()).order_by(
            "approved_go_live_at"
        )
        if dryrun:
            self.stdout.write("\n---------------------------------")
