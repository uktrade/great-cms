...
class Command(BaseCommand):
    ...
    def handle_model(self, model, model_name, model_name_plural, any_problems_fixed, options):
        ...
        if bad_depth:
            self.stdout.write(
                "Incorrect depth value found for %s: %s" % (model_name_plural, self.numberlist_to_string(bad_depth))
            )
        if bad_numchild:
            self.stdout.write(
        ...
        if any((bad_alpha, bad_path, orphans, bad_depth, bad_numchild)):
            ...
            if bad_path:
                self.stdout.write(
                    "Invalid path length found for %s: %s" % (model_name_plural, self.numberlist_to_string(bad_path))
                )
            if orphans:
