...
class TestDocumentIndexView(TestCase, WagtailTestUtils):
    ...
    def test_edit_document_link_contains_next_url(self):
        ...
        doc = models.Document.objects.create(title="Test doc", collection=evil_plans_collection)
...
class TestDocumentAddViewWithLimitedCollectionPermissions(TestCase, WagtailTestUtils):
    def setUp(self):
        add_doc_permission = Permission.objects.get(content_type__app_label="wagtaildocs", codename="add_document")
...
class TestDocumentEditView(TestCase, WagtailTestUtils):
    ...
    def test_next_url_is_present_in_edit_form(self):
        ...
        expected_next_url = reverse("wagtaildocs:index") + "?" + urlencode({"collection_id": evil_plans_collection.id})

        response = self.client.get(reverse("wagtaildocs:edit", args=(doc.id,)), {"next": expected_next_url})
        self.assertEqual(response.status_code, 200)
...
class TestMultipleCustomDocumentUploaderWithRequiredField(TestMultipleDocumentUploader):
    ...
    def test_delete_uploaded_document(self):
        ...
        """
        # Send request
        response = self.client.post(reverse("wagtaildocs:delete_upload_multiple", args=(self.uploaded_document.id,)))
...
class TestDocumentChooserView(TestCase, WagtailTestUtils):
    ...
    def test_search(self):
        response = self.client.get(reverse("wagtaildocs_chooser:choose_results"), {"q": "Hello"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_pagination(self):
        ...
        response = self.client.get(reverse("wagtaildocs_chooser:choose_results"), {"p": 2})
    ...
    def test_pagination_invalid(self):
        ...
        response = self.client.get(reverse("wagtaildocs_chooser:choose_results"), {"p": "Hello World!"})
    ...
    def test_pagination_out_of_range(self):
        ...
        response = self.client.get(reverse("wagtaildocs_chooser:choose_results"), {"p": 99999})
    ...
    def test_construct_queryset_hook_search(self):
        ...
        with self.register_hook("construct_document_chooser_queryset", filter_documents):
            response = self.client.get(reverse("wagtaildocs_chooser:choose_results"), {"q": "Test"})
        self.assertEqual(len(response.context["results"]), 1)
        self.assertEqual(response.context["results"][0], document)
...
class TestDocumentChooserChosenView(TestCase, WagtailTestUtils):
    ...
    def test_simple(self):
        response = self.client.get(reverse("wagtaildocs_chooser:chosen", args=(self.document.id,)))
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content.decode())
...
class TestGetUsage(TestCase, WagtailTestUtils):
    ...
    def test_usage_page_without_change_permission(self):
        ...
        add_permission = Permission.objects.get(content_type__app_label="wagtaildocs", codename="add_document")
        admin_permission = Permission.objects.get(content_type__app_label="wagtailadmin", codename="access_admin")
