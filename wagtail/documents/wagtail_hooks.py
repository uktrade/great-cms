...
def register_document_feature(features):
    ...
    features.register_converter_rule("editorhtml", "document-link", EditorHTMLDocumentLinkConversionRule)
    features.register_converter_rule("contentstate", "document-link", ContentstateDocumentLinkConversionRule)
...
def describe_collection_docs(collection):
    ...
    if docs_count:
        ...
        return {
            "count": docs_count,
            "count_text": ngettext("%(count)s document", "%(count)s documents", docs_count) % {"count": docs_count},
            "url": url,
        }
