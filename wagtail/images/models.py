...
class ImageQuerySet(SearchableQuerySetMixin, models.QuerySet):
    def prefetch_renditions(self, *filters):
        ...
        if filters:
            ...
            filter_specs = [filter.spec if isinstance(filter, Filter) else filter for filter in filters]
...
class AbstractImage(ImageFileMixin, CollectionMember, index.Indexed, models.Model):
    ...
    file_hash = models.CharField(max_length=40, blank=True, editable=False, db_index=True)
    ...
    def get_rendition(self, filter: Union["Filter", str]) -> "AbstractRendition":
        ...
        try:
            ...
            key = Rendition.construct_cache_key(self.id, filter.get_cache_key(self), filter.spec)
    ...
    def generate_rendition_file(self, filter: "Filter") -> File:
        ...
        output_extension = filter.spec.replace("|", ".") + IMAGE_FORMAT_EXTENSIONS[generated_image.format_name]
        if cache_key:
    ...
    def run(self, image, output):
        with image.get_willow_image() as willow:
            ...
            if "output-format" in env:
                ...
                output_format = default_conversions.get(original_format, original_format)
            ...
            raise UnknownOutputImageFormatError(f"Unknown output image format '{output_format}'")
...
class AbstractRendition(ImageFileMixin, models.Model):
    ...
    def purge_from_cache(self):
        try:
            cache = caches["renditions"]
            cache.delete(self.construct_cache_key(self.image_id, self.focal_point_key, self.filter_spec))
        except InvalidCacheBackendError:
            pass
