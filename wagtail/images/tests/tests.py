...
class TestImageTag(TestCase):
    ...
    def test_no_image_filter_provided(self):
        ...
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template("{% load wagtailimages_tags %}{% image image_obj %}")
            context = template.Context({"image_obj": self.image})
            temp.render(context)
    ...
    def test_no_image_filter_provided_when_using_as(self):
        # if image template gets the image but no filters
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template("{% load wagtailimages_tags %}{% image image_obj as foo %}")
            context = template.Context({"image_obj": self.image})
            temp.render(context)
    ...
    def test_no_image_filter_provided_but_attributes_provided(self):
        # if image template gets the image but no filters
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template('{% load wagtailimages_tags %}{% image image_obj class="cover-image"%}')
            context = template.Context({"image_obj": self.image})
            temp.render(context)
...
class TestFormat(TestCase, WagtailTestUtils):
    ...
    def test_image_to_editor_html_with_quoting(self):
        result = self.format.image_to_editor_html(self.image, 'Arthur "two sheds" Jackson')
        expected_html = (
            '<img data-embedtype="image" data-id="%d" data-format="test name" '
            'data-alt="Arthur &quot;two sheds&quot; Jackson" class="test classnames" '
            'width="640" height="480" alt="Arthur &quot;two sheds&quot; Jackson" >' % self.image.pk
        )
    ...
    def test_image_to_html_with_quoting(self):
        ...
        self.assertTagInHTML(
            '<img class="test classnames" width="640" height="480" ' 'alt="Arthur &quot;two sheds&quot; Jackson">',
            result,
            allow_extra_attrs=True,
        )
...
class TestFrontendServeView(TestCase):
    ...
    def test_get_with_extra_component(self):
        ...
        response = self.client.get(
            reverse("wagtailimages_serve", args=(signature, self.image.id, "fill-800x600")) + "test.png"
        )
    ...
    def test_get_with_too_many_extra_components(self):
        ...
        # Get the image
        response = self.client.get(
            reverse("wagtailimages_serve", args=(signature, self.image.id, "fill-800x600")) + "test/test.png"
        )
...
class TestGetImageModel(WagtailTestUtils, TestCase):
    ...
    @disconnect_signal_receiver(signal=setting_changed, receiver=update_permission_policy)
...
class TestWagtailImageField(TestCase):
    ...
    def test_to_python_with_inmemoryfile(self):
        f = WagtailImageField()
        self.image.seek(0)
        file = InMemoryUploadedFile(self.image, "", self.filename, "image/png", self.image_size, None)
    ...
    def test_to_python_gets_content_type_from_willow(self):
        f = WagtailImageField()
        self.image.seek(0)
        file = InMemoryUploadedFile(self.image, "", self.filename, "image/jpeg", self.image_size, None)
        to_python = f.to_python(file)
        self.assertIsInstance(to_python.image, WillowImageFile)
        self.assertEqual(to_python.content_type, "image/png")
    ...
    def test_to_python_with_temporary_file(self):
        f = WagtailImageField()
        with TemporaryUploadedFile("test_temp.png", "image/png", self.image_size, None) as tmp_file:
            self.image.seek(0)
