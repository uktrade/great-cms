...
class TestImage(TestCase):
    ...
    def test_file_hash(self):
        self.assertEqual(self.image.get_file_hash(), "4dd0211870e130b7e1690d2ec53c499a54a48fef")
...
class TestImageQuerySet(TestCase):
    ...
    def test_search_indexing_prefetches_tags(self):
        ...
        with self.assertNumQueries(2):
            results = {image.title: [tag.name for tag in image.tags.all()] for image in Image.get_indexed_objects()}
...
class TestPrefetchRenditions(TestCase):
    ...
    def setUp(self):
        ...
        # Generate renditions
        self.small_renditions = [image.get_rendition("max-100x100") for image in self.images]
        self.large_renditions = [image.get_rendition("min-300x600") for image in self.images]
    ...
    def test_prefetch_renditions_on_non_image_querysets(self):
        ...
        with self.assertNumQueries(3):
            ...
            pages = list(
                EventPage.objects.prefetch_related(prefetch_images_and_small_renditions).filter(
                    pk__in=self.event_pages_pks
                )
            )

        with self.assertNumQueries(0):
            # No additional query since small renditions were prefetched.
            small_renditions = [page.feed_image.get_rendition("max-100x100") for page in pages]

        self.assertListEqual(self.small_renditions, small_renditions)

        with self.assertNumQueries(3):
            # Additional queries since large renditions weren't prefetched.
            large_renditions = [page.feed_image.get_rendition("min-300x600") for page in pages]
