...
class TestEditorHtmlImageEmbedHandler(TestCase, WagtailTestUtils):
    ...
    def test_expand_db_attributes_for_editor_nonexistent_image(self):
        self.assertEqual(EditorHtmlImageEmbedHandler.expand_db_attributes({"id": 0}), '<img alt="">')
...
class TestExtractReferencesWithImage(TestCase, WagtailTestUtils):
    def test_extract_references(self):
        self.assertEqual(
            list(
                RichTextField().extract_references('<embed alt="Olivia Ava" embedtype="image" format="left" id="52"/>')
            ),
            [(Image, "52", "", "")],
        )
