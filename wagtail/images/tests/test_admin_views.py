...
class TestImageIndexView(TestCase, WagtailTestUtils):
    ...
    def test_per_page(self):
        ...
        self.assertContains(response, '<option value="60" selected="selected">60</option>', html=True)
    ...
    def test_tags(self):
        ...
        self.assertTrue([tag.name for tag in tags] == ["one", "two"] or [tag.name for tag in tags] == ["two", "one"])
    ...
    def test_tag_filtering_preserves_other_params(self):
        ...
        self.assertTrue("?p=1&amp;tag=even" in response_body or "?tag=even&amp;p=1" in response_body)
        # next link should exist and include tag
        self.assertTrue("?p=3&amp;tag=even" in response_body or "?tag=even&amp;p=3" in response_body)
...
class TestImageAddViewWithLimitedCollectionPermissions(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        user = self.create_user(username="moriarty", email="moriarty@example.com", password="password")
    ...
    def test_add(self):
        ...
        self.assertTrue(Image.objects.filter(title="Test image").exists())
        self.assertEqual(Image.objects.get(title="Test image").collection, self.evil_plans_collection)
...
class TestImageEditView(TestCase, WagtailTestUtils):
    ...
    def test_next_url_is_present_in_edit_form(self):
        ...
        expected_next_url = (
            reverse("wagtailimages:index") + "?" + urlencode({"collection_id": evil_plans_collection.id})
        )
    ...
    def test_reupload_same_name(self):
        ...
        new_file = SimpleUploadedFile(new_name, get_test_image_file(colour="red").file.getvalue())
    ...
    def test_unique_together_validation_error(self):
        ...
        # another image with a title to collide with
        CustomImage.objects.create(title="Edited", file=get_test_image_file(), collection=evil_plans_collection)
...
class TestUsage(TestCase, WagtailTestUtils):
    ...
    def test_usage_page(self):
        ...
        response = self.client.get(reverse("wagtailimages:image_usage", args=[self.image.id]))
...
class TestImageChooserChosenView(TestCase, WagtailTestUtils):
    ...
    def get(self, params={}):
        return self.client.get(reverse("wagtailimages_chooser:chosen", args=(self.image.id,)), params)
...
class TestImageChooserUploadView(TestCase, WagtailTestUtils):
    ...
    def test_upload_duplicate(self):
        ...
        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "wagtailimages/chooser/confirm_duplicate_upload.html")
        ...
        choose_new_image_action = reverse("wagtailimages_chooser:chosen", args=(new_image.id,))
        self.assertEqual(response.context["confirm_duplicate_upload_action"], choose_new_image_action)

        choose_existing_image_action = (
            reverse("wagtailimages:delete", args=(new_image.id,))
            + "?"
            + urlencode({"next": reverse("wagtailimages_chooser:chosen", args=(existing_image.id,))})
        )
        self.assertEqual(
    ...
    def test_upload_duplicate_select_format(self):
        ...
        choose_new_image_action = (
            reverse("wagtailimages_chooser:select_format", args=(new_image.id,)) + "?select_format=true"
        )
        self.assertEqual(response.context["confirm_duplicate_upload_action"], choose_new_image_action)
        ...
        choose_existing_image_action = (
            ...
            + urlencode(
                {
                    "next": reverse("wagtailimages_chooser:select_format", args=(existing_image.id,))
                    + "?select_format=true"
                }
            )
        )
...
class TestMultipleImageUploader(TestCase, WagtailTestUtils):
    ...
    def test_add_post_duplicate(self):
        ...
        # Check template used
        self.assertTemplateUsed(response, "wagtailimages/images/confirm_duplicate_upload.html")
    ...
    def test_add_post_duplicate_choose_permission(self):
        ...
        # Check template used
        self.assertTemplateNotUsed(response, "wagtailimages/images/confirm_duplicate_upload.html")
...
class TestMultipleImageUploaderWithCustomRequiredFields(TestCase, WagtailTestUtils):
    ...
    def test_add_post(self):
        ...
        self.assertEqual(
            response.context["edit_action"],
            "/admin/images/multiple/create_from_uploaded_image/%d/" % response.context["uploaded_image"].id,
        )
        self.assertEqual(
            response.context["delete_action"],
            "/admin/images/multiple/delete_upload/%d/" % response.context["uploaded_image"].id,
        )
    ...
    def test_delete_uploaded_image(self):
        ...
        """
        # Send request
        response = self.client.post(reverse("wagtailimages:delete_upload_multiple", args=(self.uploaded_image.id,)))
