        ...
        if options["purge_only"]:
            for rendition in renditions:
                try:
                    ...
                    self.stderr.write(f"Could not purge rendition for {rendition_image.title}")
            self.stdout.write(self.style.SUCCESS(f"Successfully purged {success_count} image rendition(s)"))
        else:
            for rendition in renditions:
                try:
                    ...
                    self.stderr.write(f"Could not regenerate rendition for {rendition_image.title}")
            self.stdout.write(self.style.SUCCESS(f"Successfully regenerated {success_count} image rendition(s)"))
