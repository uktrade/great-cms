...
class BaseImageChooseView(BaseChooseView):
    ...
    def get_object_list(self):
        return (
            permission_policy.instances_user_has_any_permission_for(self.request.user, ["choose"])
    ...
    def collections(self):
        collections = self.permission_policy.collections_user_has_permission_for(self.request.user, "choose")
    ...
    def get_context_data(self, **kwargs):
        ...
        for image in context["results"]:
            image.chosen_url = self.append_preserved_url_parameters(reverse(chosen_url_name, args=(image.id,)))
...
class ImageUploadViewMixin(SelectFormatResponseMixin, CreateViewMixin):
    ...
    def render_duplicate_found_response(self, request, new_image, existing_image):
        ...
        choose_new_image_url = self.append_preserved_url_parameters(reverse(next_step_url, args=(new_image.id,)))
...
class ImageSelectFormatView(SelectFormatResponseMixin, ImageChosenResponseMixin, View):
    ...
    def get_chosen_response_data(self, image):
        ...
        response_data = super().get_chosen_response_data(image, preview_image_filter=format.filter_spec)
