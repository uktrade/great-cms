...
def get_rewriter():
    ...
    return MultiRuleRewriter(
        [
            LinkRewriter(
                {linktype: handler.expand_db_attributes for linktype, handler in link_rules.items()},
                {linktype: handler.extract_references for linktype, handler in link_rules.items()},
            ),
            EmbedRewriter(
                {embedtype: handler.expand_db_attributes for embedtype, handler in embed_rules.items()},
                {linktype: handler.extract_references for linktype, handler in embed_rules.items()},
            ),
        ]
    )
