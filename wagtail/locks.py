...
class BasicLock(BaseLock):
    ...
    def for_user(self, user):
        ...
        if global_edit_lock is None and hasattr(settings, "WAGTAILADMIN_GLOBAL_PAGE_EDIT_LOCK"):
            warn(
        ...
        if self.object.locked_by_id == user.pk:
            if self.object.locked_at:
                if self.is_page:
                    return format_html(
                        ...
                        _("<b>Page '{page_title}' was locked</b> by <b>you</b> on <b>{datetime}</b>."),
                        page_title=title,
                    ...
                    return format_html(
                        # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                        _("<b>'{title}' was locked</b> by <b>you</b> on <b>{datetime}</b>."),
                        title=title,
                        datetime=self.object.locked_at.strftime("%d %b %Y %H:%M"),
                    )
            ...
            if self.object.locked_by and self.object.locked_at:
                if self.is_page:
                    return format_html(
                        # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                        _("<b>Page '{page_title}' was locked</b> by <b>{user}</b> on <b>{datetime}</b>."),
                        page_title=title,
                    ...
                    return format_html(
                        # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                        _("<b>'{title}' was locked</b> by <b>{user}</b> on <b>{datetime}</b>."),
                        title=title,
                        user=str(self.object.locked_by),
                        datetime=self.object.locked_at.strftime("%d %b %Y %H:%M"),
                    )
            ...
            if current_workflow_state and len(current_workflow_state.all_tasks_with_status()) == 1:
                ...
                if self.is_page:
                    workflow_info = format_html(
                        # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                        _("This page is awaiting <b>'{task_name}'</b> in the <b>'{workflow_name}'</b> workflow."),
...
class ScheduledForPublishLock(BaseLock):
    ...
    def get_message(self, user):
        ...
        if self.is_page:
            return format_html(
                # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                _("Page '{page_title}' is locked and has been scheduled to go live at {datetime}"),
            ...
            message = format_html(
                # nosemgrep: translation-no-new-style-formatting (new-style only w/ format_html)
                _("{model_name} '{title}' is locked and has been scheduled to go live at {datetime}"),
                model_name=self.object._meta.verbose_name,
